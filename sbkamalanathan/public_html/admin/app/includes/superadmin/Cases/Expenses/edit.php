...
<?php                                                                                
    ...
    $edit_data = $mysql->select("select * from _tbl_cases_expenses where md5(concat(CreatedOn,CaseExpenseID))='".$_GET['edit']."'");
    if (isset($_POST['CreateBtn'])) {
        ...
        $target_dir.=$CaseDetails[0]['CaseID'];
         ...
         if (!(is_dir($target_dir."/Expenses"))) {
            mkdir($target_dir."/Expenses");
        }
        $target_dir.="/Expenses/";
        ...
        if ($error==0) {
            ...
            $mysql->execute("update _tbl_cases_expenses set 
                                                               TxnDate        = '".trim($_POST['TxnDate'])."',
                                                               Amount         = '".trim($_POST['Amount'])."',
                                                               Description    = '".trim($_POST['Description'])."'  where md5(concat(CreatedOn,CaseExpenseID))='".$_GET['edit']."'");
          if ($Attachment_FileName!="") {
                
                $mysql->insert("_tbl_cases_attachments",array("CaseID"                 => $CaseDetails[0]['CaseID'],
                                                             "AttachmentFor"          => "Expenses",
                                                             "RecordID"               => $edit_data[0]['CaseExpenseID'],
                                                             "FileName"               => $FileName,
            ...
            <?php
               ...
               $mysql->execute("update _tbl_cases_expenses set IsActive          = '2',
                                                                       ...
                                                                       where md5(concat(CreatedOn,CaseExpenseID))='".$_GET['edit']."'");
               unset($_POST);
               ?>
                   <div class="row">
                <div class="col-12">
              ...
              <div class="alert alert-success outline alert-dismissible fade show" role="alert" style="margin-bottom: 0px;">
                      ...
                      <br>
                      <a href="dashboard.php?action=Cases/view&sa=Expenses/list&case=<?php echo $_GET['case'];?>" class="btn btn-success">Continue</a>
                    </div>
                    </div>
              </div>
              </div>
  ...
  <form id="frmD" action="" method="post" id="create_case_amountreceved" name="create_case_amountreceved" onsubmit="return formvalidation('create_case_amountreceved');" enctype="multipart/form-data">
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body">
                    <h6 style="margin-bottom:30px">Edit Expenses information</h6>
                        ...
                        <div class="row g-3  mb-3">
                         ...
                         <div class="row g-3  mb-3">
                            ...
                            <input type="hidden" value="0" id="isDelete" name="isDelete">
                                <a href="dashboard.php?action=Cases/view&sa=Expenses/list&case=<?php echo $_GET['case'];?>" class="btn btn-outline-primary">Back</a>
