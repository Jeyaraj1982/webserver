<?php 
    ...
    $documents = $mysql->select("select * from _tbl_cases_receviedpayments where CaseID='".$CaseDetails[0]['CaseID']."' and  IsActive=1 order by CasePaymentID desc");
    $documents_amount = $mysql->select("select sum(Amount) as Amount from _tbl_cases_receviedpayments where CaseID='".$CaseDetails[0]['CaseID']."' and  IsActive=1 order by CasePaymentID desc");
?>
<div class="card">
    <div class="card-header">
        <div class="row">
            <div class="col-md-6">
                <h5 style="margin-top:5px">Amount Recevied</h5>
                <h6 style="clear:both;margin-top: 8px;color: #999;font-weight: normal;font-size: 14px;">Total Recevied: <?php echo number_format((isset($documents_amount[0]['Amount']) ? $documents_amount[0]['Amount'] : 0),2);?></h6>
            </div>
            <div class="col-md-6 text-end">
                 <div class="media">
                    <div class="media-body text-end">
                        <div class="btn btn-primary" onclick="location.href='dashboard.php?action=Cases/view&sa=AmountRecevied/add&case=<?php echo $_GET['case'];?>'"><i data-feather="plus-square"></i>Add</div>
                    </div>
                 </div>
            </div>
        </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="text-align: right;" class="jsgrid jsgrid-cell jsgrid-control-field jsgrid-align-center">
                                    ...
                                    <a style="color:red" href="dashboard.php?action=Cases/view&sa=AmountRecevied/edit&case=<?php echo $_GET['case'];?>&edit=<?php echo md5($document['CreatedOn'].$document['CasePaymentID']);?>"> 
