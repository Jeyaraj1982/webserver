<?php
    if (isset($_POST['btnsubmit'])) {
        $ErrorCount =0;
            $dupStaffCode = $mysql->select("select * from _queen_staffs where StaffCode='".$_POST['StaffCode']."'");
            if(sizeof($dupStaffCode)>0){
                $ErrStaffCode ="Staff Code Already Exist";
                $ErrorCount++;
            }
            $dupSurName = $mysql->select("select * from _queen_staffs where SurName='".$_POST['SurName']."'");
            ...
            if($ErrorCount==0){
              ...
              $id = $mysql->insert("_queen_staffs",array("StaffCode"          => $_POST['StaffCode'],
                                ...
                                <form id="exampleValidation" method="POST" action="" onsubmit="return SubmitBrand();" id="addStaff" enctype="multipart/form-data">
                                   <div class="card-body">
										<div class="form-group form-show-validation row">
                                            ...
                                            <input type="text" class="form-control" id="StaffCode" name="StaffCode" placeholder="Enter Staff Code" value="<?php echo (isset($_POST['StaffCode']) ? $_POST['StaffCode'] :"");?>">
