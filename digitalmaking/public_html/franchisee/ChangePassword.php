<?php
    if (isset($_POST['btnsubmit'])) {
        $data = $mysql->select("select * from `_tbl_franchisee` where `FranchiseeID`='".$_SESSION['User']['FranchiseeID']."'");
        if($data[0]['Password']==$_POST['CurrentPassword']){
            $mysql->execute("update `_tbl_franchisee` set `Password`='".$_POST['confirmnewpassword']."' where `FranchiseeID`='".$_SESSION['User']['FranchiseeID']."'"); 
            unset($_POST);
