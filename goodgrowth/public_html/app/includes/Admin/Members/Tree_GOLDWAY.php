...
<div class="heading1"><span>GOLD WAY</span></div>
<?php
    if (isset($_GET['msg'])) {
           echo SuccessMsg($_GET['msg']);        
    }
    
    $loginEntries = $mysql->select("select * from _tbl_Members where MemberID in (select ChildID from _tbl_Member_Placement where Plan='1' and PlacedBy='".$mem[0]['MemberID']."') order by MemberID desc");
...
<br><br>
 <?php
 if ($mem[0][$PlanGrade]==1) {
    $newClass = "";
    $lData = $mysql->select("select * from _tbl_Member_Placement where ParentID='".$mem[0]['MemberID']."' and ChildPosition='L' and Plan='".$PlanID."'");
    $rData = $mysql->select("select * from _tbl_Member_Placement where ParentID='".$mem[0]['MemberID']."' and ChildPosition='R' and Plan='".$PlanID."'");
 ...
 ?>
        ...
        <table style="width:800px;margin:0px auto;background:#eddd97" cellpadding="5" cellspacing="5">
            <tr>
                    ...
                    <?php
                      if (sizeof($lData)>0) {
                       $lDataSuNodeL=$mysql->select("select * from _tbl_Member_Placement where Plan=".$PlanID." and ParentID='".$lData[0]['ChildID']."' and ChildPosition='L'");
                        $newClass = ($lDataSuNodeL[0]['PlacedBy']==$mem[0]['MemberID']) ? " border " : "";
                ...
                <td style="width:200px;padding-bottom:10px;text-align:center;">
                    <?php
                     if (sizeof($lData)>0) {
                        ...
                        $newClass = ($lDataSuNodeR[0]['PlacedBy']==$mem[0]['MemberID']) ? " border " : "";
                ...
                <td style="width:200px;padding-bottom:10px;text-align:center;">
                    ...
                    if (sizeof($rData)>0) {
                        ...
                        $newClass = ($rDataSuNodeL[0]['PlacedBy']==$mem[0]['MemberID'])  ? " border " :"" ; 
                    ...
                    <?php
                     if (sizeof($rData)>0) {
                        ...
                        if (sizeof($rDataSuNodeR)==1) {
                            ...
                            $newClass = ($rDataSuNodeR[0]['PlacedBy']==$mem[0]['MemberID'])  ? " border " :"" ; 
            ...
            <tr>
                <td colspan="4"> <div class="Gold"><?php echo $mem[0]['FirstName']." ".$mem[0]['LastName']."<br>".$mem[0]['MemberCode']; ?></div> </td>
            </tr>
        </table>
 <?php } else { 
 ...
 <?php
    if ($PlanComplete==6) {
        ...
        $ud = $mysql->select("select * from _tbl_Members   where MemberID='".$mem[0]['MemberID']."'");
        
        if ($ud[0]['GoodwayCompleted']>0) {
            
        } else {
            $mysql->execute("update _tbl_Members set   PlanUpgradedB='1', GoodwayCompleted='1',GoodWayID='".implode(".",$PlanCompletedIDs)."' where MemberID='".$mem[0]['MemberID']."'");    
        }
