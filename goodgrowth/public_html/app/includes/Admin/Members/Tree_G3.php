...
<?php
 if (isset($_GET['msg'])) {
    ...
    $loginEntries = $mysql->select("select * from _tbl_Members where MemberID in (select ChildID from _tbl_Member_Placement where Plan='4' and PlacedBy='".$mem[0]['MemberID']."') order by MemberID desc");
                ...
                <?php
                    if (sizeof($level1_2)>0) {  
                        ...
                        if (sizeof($level1_2D)==1) {
                            ...
                            $newClass = ($level1_2D[0]['PlacedBy']==$mem[0]['MemberID']) ? " border " : "";
 ...
 <?php
    if ($PlanComplete==6) {
        ...
        if ($ud[0]['G3Completed']>0) {
            ...
            $mysql->execute("update _tbl_Members set   PlanUpgradedA='1', G3Completed='1',G3ID='".implode(".",$PlanCompletedIDs)."' where MemberID='".$mem[0]['MemberID']."'");    
        }
