    ...
    $('#mymembers').text(Math.ceil(this.Counter));
        ...
        <td style="width:2%">&nbsp;</td>   -->
         ...
         <td class="dash_theme_widget dash_theme_gray">
            ...
            <?php
                $invoice = $mysql->select("select sum(InvoiceAmount) as InvoiceAmount from _tbl_Accounts_Invoices");
        ...
        <td class="dash_theme_widget dash_theme_gray">
            ...
            <?php
                $Receipt = $mysql->select("select sum(ReceiptAmount) as ReceiptAmount from _tbl_Accounts_Receipts");
        ...
        <td class="dash_theme_widget dash_theme_gray">
            ...
            <div class="dash_widget_Count">
            <?php
                 echo convertcash($Receipt[0]['ReceiptAmount']-$invoice[0]['InvoiceAmount'])   ;
                ?>
            </div>
        </td>
        <td style="width:2%">&nbsp;</td>
        ...
        <td class="dash_theme_widget dash_theme_gray">
            <div class="dash_widget_title">Web Orders</div>
            <div class="dash_widget_Count">
            <?php echo sizeof($mysql->select("select * from _tb_Orders order by OrderID desc")); ?>
            </div>
        </td>
        <td style="width:2%">&nbsp;</td>
...
if (isset($_POST['btnGraph'])) {
    ...
    $days = 4;
     ...
     $invoice = $mysql->select("select sum(InvoiceAmount) as amt from _tbl_Accounts_Invoices where date(TxnDate)=date('".date('Y-m-d',strtotime("-$i day"))."')");
     $receipt = $mysql->select("select sum(ReceiptAmount) as amt from _tbl_Accounts_Receipts where date(TxnDate)=date('".date('Y-m-d',strtotime("-$i day"))."')");
...
$datasets_receipts = substr($datasets_receipts,0,strlen($datasets_receipts)-1);
   ...
   <!-- <button id="randomizeData">Randomize Data</button>
    ...
    <script>
        ...
        <td></td>
        <td colspan="3" class="dash_theme_gray">
    ...
    <tr>
        ...
        <td class="dash_theme_widget dash_theme_gray">
            ...
            <div class="dash_widget_Count">
            <?php
                $emailCount  = $mysql->select("select MemberID from _tbl_Log_Emails");
                echo sizeof($emailCount);
            ?>
            </div>
            <br>
            Available Mail Credits: <?php echo 10000-sizeof($emailCount);?>
        </td>
        <td style="width:2%">&nbsp;</td>
        ...
        <td class="dash_theme_widget dash_theme_gray">
            ...
            <div class="dash_widget_Count">
            <?php
                $smsCount  = $mysql->select("select MemberID from _tbl_Log_MobileSMS");
                echo sizeof($smsCount);
            ?>
            </div>
            <br>
            Available SMS Credits: <?php echo 10000-sizeof($smsCount);?>
        </td>
    </tr>
    <tr>
        <td colspan="11">&nbsp;</td>
    ...
    <tr>
        ...
        <br>
            <table style="width:100%">
             <?php 
                $recentMembes = $mysql->select("select * from _tbl_Members where PlanID='4' order by MemberID Desc limit 0,5 ");
                foreach($recentMembes as $rem) {
                    ?>
                    <tr>
                        <td colspan="2"><?php echo ucwords(strtolower($rem['FirstName']));?> (<span style="color:Green"><?php echo $rem['MemberCode'];?></span>)</td>
        ...
        <br>
            <table style="width:100%">
             ...
             <?php 
                $recentMembes = $mysql->select("select * from _tbl_Members where PlanID='1' order by MemberID Desc limit 0,5 ");
                foreach($recentMembes as $rem) {
                    ?>
                    <tr>
                        <td colspan="2"><?php echo ucwords(strtolower($rem['FirstName']));?> (<span style="color:Green"><?php echo $rem['MemberCode'];?></span>)</td>
        ...
        <br>
            <table style="width:100%">
             <?php                                                                                                                                             
                $data =  $mysql->select("select * from _tbl_Members where MemberID in (select MemberID from _tbl_Member_Attachment where AttachmentType='IDPROOF' and IsApproved='0' and IsRejected='0') and ReferedBy>0 order by MemberID desc");
                foreach($data as $rem) {
                    ?>
                    <tr>
                        <td colspan="2"> <?php echo  $rem['FirstName'];?> (<span style="color:green"><?php echo  $rem['MemberCode'];?></span>) </td>
                    </tr>
                    <tr>
                        <td colspan="2"><hr style="border:none;border-bottom:1px solid #f1f1f1"></td>
                    </tr>
    ...
    <tr>
        ...
        <br>
            <table style="width:100%">
             ...
             <?php 
                $recentMembes = $mysql->select("select * from _tbl_Members where PlanID='2' order by MemberID Desc limit 0,5 ");
                foreach($recentMembes as $rem) {
                    ?>
                    <tr>
                        <td colspan="2"><?php echo ucwords(strtolower($rem['FirstName']));?> (<span style="color:Green"><?php echo $rem['MemberCode'];?></span>)</td>
        ...
        <br>
            <table style="width:100%">
             ...
             <?php 
                $recentMembes = $mysql->select("select * from _tbl_Members where PlanID='3' order by MemberID Desc limit 0,5 ");
                foreach($recentMembes as $rem) {
                    ?>
                    <tr>
                        <td colspan="2"><?php echo ucwords(strtolower($rem['FirstName']));?> (<span style="color:Green"><?php echo $rem['MemberCode'];?></span>)</td>
