           ...
           $data = $mysql->select("Select * from `_tbl_verification_code` where ID='".$_POST['reqID']."'");
             if (sizeof($data)>0) {
                 if ($data[0]['SecurityCode']==$_POST['SequirityCode']) {   ?>
                    <form action="<?php echo path_url;?>FpwdNewPassword" id="reqFrm" method="post">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#SequirityCode").blur(function() {   
            ...
            var email = $('#SequirityCode').val().trim();
            if (email.length==0) {
                $('#ErrSequirityCode').html("Please Enter Verification Code");   
            } 
        });
    });
    ...
    var email = $('#SequirityCode').val().trim();
        if (email.length==0) {
            $('#ErrSequirityCode').html("Please Enter Verfication Code");   
            ErrorCount++;      
        }
        if (ErrorCount>0) {
            return false;
           ...
           <a href="<?php echo country_url;?>" class="close-quick-sidebar" style="float: right;font-size:20px;margin-top:-40px;color:#ccc">
            ...
            <div class="login-form">
                <div class="form-group form-floating-label">
                    <input id="SequirityCode" name="SequirityCode" value="<?php echo isset($_POST['SequirityCode']) ? $_POST['SequirityCode'] : '';?>" type="text" class="form-control input-border-bottom" required>
                    <label for="SequirityCode" class="placeholder">Verification Code</label>
                    <br> <span class="errorstring" id="ErrSequirityCode" style="color:red;font-size: 11px;"><?php echo $ErrSequirityCode;?></span>
                </div>
