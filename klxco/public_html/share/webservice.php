                    ...
                    $html .= '<div class="col-sm-12">';
                        $html .= '<div class="form-group row">';
                                $html .= '<div class="col-sm-6"><label>From Year</label>';
                                    $html .='<input type="text" class="form-control" name="Year" id="Year" placeholder="Year">';
                        ...
                        $html .='</div>';
                        $html .= '<div class="form-group row">';
                            $html .= '<div class="col-sm-12"><label>Job Title</label>';
                                $html .='<input type="text" class="form-control" name="Title" id="Title" placeholder="Job Title">';
       ...
       $html = '<form action="" method="POST" id="ExperienceFrm'.$_REQUEST['ExperienceID'].'">';
            ...
            $html .='<div class="modal-body">';
                $html .= '<div class="form-group row">';
                    $html .= '<div class="col-sm-12">';
                        $html .= '<div class="form-group row">';
                            $html .= '<div class="col-sm-12"><label>Year</label>';
                                $html .='<input value="'.$Experience[0]['Year'].'" type="text" class="form-control" name="Year" id="Year" placeholder="Year">';
                                $html .='<div id="ErrorYear" style="font-size:12px;color:red"></div>';
                            $html .='</div>';
                        $html .='</div>';
                        $html .= '<div class="form-group row">';
                        ...
                        $html .= '<div class="form-group row">';
                            $html .= '<div class="col-sm-12"><label>Job Title</label>';
                                $html .='<input value="'.$Experience[0]['Title'].'" type="text" class="form-control" name="Title" id="Title" placeholder="Job Title">';
                                $html .='<div id="ErrorTitle" style="font-size:12px;color:red"></div>';
                            $html .='</div>';
                        $html .='</div>';
                        $html .= '<div class="form-group row">';
...
function UpdateExperience(){
    global $mysql;
     ...
     $mysql->execute("update _tbl_resume_experience set `Year`   ='".$_POST['Year']."',
                                                       `Designation`         ='".$_POST['Designation']."',
                                                       `Title`         ='".$_POST['Title']."',
                                                       `NameofCompany`      ='".$_POST['NameofCompany']."',
                                                       `WorkingPlace`      ='".$_POST['WorkingPlace']."',
...
function AddHobbies(){
    global $mysql;
    $id = $mysql->insert("_tbl_resume_hobbies",array("ResumeID"     => $_POST['ResumeID'],
                                                        "Title"        => $_POST['Title'],
                                                        "Description"   => $_POST['Description'],
                                                        "CreatedOn"     => date("Y-m-d H:i:s")));
...
function EditHobbiesDetails(){
       global $mysql;
       $Hobbies = $mysql->select("select * from _tbl_resume_hobbies where HobbiesID='".$_REQUEST['HobbiesID']."'");
       $html = '<form action="" method="POST" id="HobbiesFrm'.$_REQUEST['HobbiesID'].'">';
            $html.= '<input type="hidden" value="'.$_REQUEST['HobbiesID'].'" id="HobbiesID" Name="HobbiesID">';
            ...
            $html .='<div class="modal-body">';
                $html .= '<div class="form-group row">';
                    $html .= '<div class="col-sm-12">';
                        $html .= '<div class="form-group row">';
                            $html .= '<div class="col-sm-12"><label>Title</label>';
                                $html .='<input value="'.$Hobbies[0]['Title'].'" type="text" class="form-control" name="Title" id="Title" placeholder="Title">';
                                $html .='<div id="ErrorTitle" style="font-size:12px;color:red"></div>';
                            $html .='</div>';
                        $html .='</div>';
                        $html .= '<div class="form-group row">';
       ...
       $html .='<div class="modal-footer">';
            $html .='<button type="button" class="btn btn-outline-success" data-dismiss="modal" >Cancel</button>&nbsp;&nbsp;&nbsp;';
            $html .='<button type="button" class="btn btn-success" onclick="UpdateHobbies('.$_REQUEST['HobbiesID'].')" >Update</button>';
       $html .='</div>';
       $html .='</form>';
       return $html;                                                                                
    }
function UpdateHobbies(){
    global $mysql;
     ...
     $mysql->execute("update _tbl_resume_hobbies set `Title`         ='".$_POST['Title']."',
                                                       `Description`    ='".$_POST['Description']."'
                                                        where HobbiesID  ='".$_POST['HobbiesID']."'");
...
function DeleteHobbies(){
    global $mysql;
     $Skills = $mysql->select("select * from _tbl_resume_hobbies where HobbiesID='".$_POST['HobbiesID']."'");
     $mysql->execute("DELETE FROM _tbl_resume_hobbies  where HobbiesID  ='".$_POST['HobbiesID']."'");
...
function DeleteResume(){
    global $mysql;
     ...
     $mysql->execute("DELETE FROM _tbl_resume_general_info  where ResumeID  ='".$_POST['ResumeID']."'");
     $mysql->execute("DELETE FROM _tbl_resume_education  where ResumeID  ='".$General[0]['ResumeID']."'");
     $mysql->execute("DELETE FROM _tbl_resume_experience  where ExperienceID  ='".$General[0]['ResumeID']."'");
