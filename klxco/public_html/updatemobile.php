     ...
     include_once("config.php"); 
       $obj = new CommonController();
         ...
         if (isset($_POST['loginbtn'])) {
          ...
          if ($_POST['mobilenumber']<9999999999 && $_POST['mobilenumber']>6000000000)    {
              $record = $mysql->select("select * from _jusertable where mobile='".trim($_POST['mobilenumber'])."' and userid<>'".$_SESSION['USER']['userid']."'");
              if (sizeof($record)==0) {
                  $mysql->execute("update _jusertable set mobile='".$_POST['mobilenumber']."',countryid='1',countryname='India' where userid='".$_SESSION['USER']['userid']."'");
                  $record = $mysql->select("select * from _jusertable where  userid='".$_SESSION['USER']['userid']."'");
                  $_SESSION['USER'] = $record[0];
              //    echo " <script>location.href='updatelocation.php';</script>";
                  echo " <script>location.href='mypage.php?s=1';</script>";
              } else {
                 echo $obj->printError("Mobile number already in use");
              }
          } else {
...
<body class="login">
    <div class="wrapper wrapper-login">
        <div class="container container-login animated fadeIn">
            <h3 class="text-center">
                ...
                <br>Update your mobile number</h3>
            <form action="" method="POST" onsubmit="return doConfrim();">
            <div class="login-form">
                
                <div class="form-group row form-floating-label">
                    <div class="col-sm-5">
                        <select class="form-control" style="padding-left:5px;padding-right: 0px;border:none;border-bottom: 1px solid #ebedf2;"> 
                            <option value="1">India</option>
                        </select>    
                     </div>
                    <div class="col-sm-7">
                        <input id="mobilenumber" name="mobilenumber" value="<?php echo $_POST['mobilenumber'];?>" maxlength="10" type="text" class="form-control input-border-bottom">
                        <label for="username" class="placeholder">Mobile Number</label>
                        <span class="errorstring" id="Errmobilenumber" style="font-size:11px;color:red;"></span>
                    </div>
                </div>
                <div class="form-action mb-3">
                    <button type="submit"  name="loginbtn"class="btn btn-primary btn-rounded btn-login" style="width:auto;padding:4px 20px;">Update Mobile Number</button>
                </div>
                <div class="form-action mb-3">
    ...
    <script src="https://www.klx.co.in/assets/js/js/jquery-ui.min.js"></script>
    <script src="https://www.klx.co.in/assets/js/js/popper.min.js"></script>
    <script src="https://www.klx.co.in/assets/js/js/bootstrap.min.js"></script>
