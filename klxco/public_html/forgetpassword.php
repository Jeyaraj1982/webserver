     ...
     include_once("config.php"); 
       $obj = new CommonController();
        if (isset($_POST['UserName'])) {
            $ErrorCount =0;
            $data = $mysql->select("select * from _jusertable where email='".$_POST['UserName']."'");
            if (sizeof($data)==0){
                $ErrUserName ="Email Address Not Available";
                $ErrorCount++;
            }
            if($ErrorCount==0){
                 $otp=rand(1000,9999);
                 $securitycode = $mysql->insert("_tbl_verification_code",array("UserID"        => $data[0]['userid'],
                 ...
                 $message = '
                    ...
                    $mparam['MailTo']=$data[0]['email'];
                    $mparam['MemberID']=$data[0]['userid'];
                    $mparam['Subject']="KLX :) Forget password";
                    $mparam['Message']=$message;
                    MailController::Send($mparam,$mailError);   
                    ?>                                                                                  
                    <form action="<?php echo path_url;?>FpwdOTP" id="reqFrm" method="post">
                        <input type="hidden" value="<?php echo $securitycode;?>" name="reqID">
                        <input type="hidden" value="<?php echo $data[0]['email'];?>" name="reqEmail">
                    </form>
...
function IsEmail(email) {
    ...
    $(document).ready(function(){
        ...
        $("#UserName").blur(function() {   
            ...
            if (email.length==0) {
                $('#ErrUserName').html("Please Enter Email Address");   
            } else {
                if (!(IsEmail(email))) {
                    $('#ErrUserName').html("Please Enter Valid Email Address");
                }
            }
        });
    });
           ...
           <a href="<?php echo country_url;?>" class="close-quick-sidebar" style="float: right;font-size:20px;margin-top:-40px;color:#ccc">
            ...
            <div class="login-form">
                <div class="form-group form-floating-label">
                    <input id="UserName" name="UserName" value="<?php echo isset($_POST['UserName']) ? $_POST['UserName'] : '';?>" type="text" class="form-control input-border-bottom" required>
                    <label for="username" class="placeholder">Email Address</label>
                    <br> <span class="errorstring" id="ErrUserName" style="color:red;font-size: 11px;"><?php echo $ErrUserName;?></span>
                </div>
                ...
                <?php
                  ...
                  ?> 
                    <button type="submit"  name="loginbtn" class="btn btn-primary btn-rounded btn-login">Submit</button>
                </div>
                <div class="login-account">
                    <a href="<?php echo path_url;?>login" id="show-signup" class="link">Back to signin</a>
                </div>
            </div>
            </form>
        </div>
