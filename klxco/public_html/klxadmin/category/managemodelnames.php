...
function ConfirmationEditModelName(modelid, modelname){
 ...
 function doUpdate(modelid) {
    ...
    if(ErrorCount==0){
        ...
        $.post( "https://klx.co.in/webservice.php?action=UpdateModelName",  param,function( data ) {
...
function ConfirmationDeleteModelName(modelid, modelname){
 ...
 function doDelete(modelid) {
        ...
        $.post( "https://klx.co.in/webservice.php?action=deleteModelName",  param,function( data ) {
            var obj = JSON.parse(data); 
            var html = "";
            if (obj.status=="failure") {
