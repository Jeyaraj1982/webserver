<?php
    if (isset($_POST['btnsubmit'])) {
        $data = $mysql->select("select * from `_jusertable` where `userid`='".$_SESSION['USER']['userid']."'");
        if($data[0]['pwd']==$_POST['CurrentPassword']){
            $mysql->execute("update `_jusertable` set `pwd`='".$_POST['confirmnewpassword']."' where `userid`='".$_SESSION['USER']['userid']."'"); 
            unset($_POST);
