...
function ConfirmationDeleteCity(citynameid,cityname){
 ...
 function doUpdateCityName(citynameid) {
    ...
    if(ErrorCount==0){
        ...
        $.post( "https://klx.co.in/webservice.php?action=UpdateCityName",  param,function( data ) {
...
function doDeleteCityName(citynameid) {
    var param = $( "#CityFrm_"+citynameid).serialize();
    $("#confrimation_text").html(loading);
    $.post( "https://klx.co.in/webservice.php?action=deleteCityName",param,function(data) {
        var obj = JSON.parse(data); 
        if (obj.status=="failure") {
            html = "<div style='width:100%;padding:10px;background:#fff;text-align:center;color:red'><img src='https://klx.co.in/klxadmin/assets/accessdenied.png' style='width:128px'><br><br>Transaction failed.<br>"+obj.message;
        } else { 
