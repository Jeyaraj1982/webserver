...
<?php 
  $sql= $mysql->select("select * from `_tbl_withdrawal_request` where  `RequestID`='".$_GET['ReqID']."'");
?>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card">
                                ...
                                <input type="hidden" value="<?php echo $sql[0]['FranchiseeID'];?>" id="FranchiseeID" Name="FranchiseeID"    >
                                ...
                                <input type="hidden" value="<?php echo $sql[0]['Amount'];?>" id="Amount" Name="Amount"    >
                                    <div class="card-body">
                                        <div class="form-group form-show-validation row">
                                            <label for="MobileNumber" class="col-lg-3 col-md-3 col-sm-4 mt-sm-2 text-left">Withdrawal Amount </label>
                                        ...
                                        <div class="form-group form-show-validation row">
                                            ...
                                            <div class="col-lg-4 col-md-9 col-sm-8"><?php echo $sql[0]['TxnOn'];?></div>
                                        </div>
                                        ...
                                        <?php if($sql[0]['Status']==2){    ?>
                                          <div class="form-group form-show-validation row">
                                                ...
                                                Rejected On : <?php echo $sql[0]['RejectedOn'];?></span>
                                            </div>
                                        <?php } ?>
                                    </div>
...
<script>
 ...
 function DoApprove(RequestID) {
    var buttons = "<div style='text-align:center;padding:10px;'><button type='button' class='btn btn-secondary btn-sm' onclick='location.href=\"dashboard.php\"'>Home</button>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='location.href=\"dashboard.php?action=Requests/FranchiseeWithdrawalRequests&filter=Paid\"'>Continue</button></div>";
    $("#confrimation_text").html(loading);
    
    $.post( "https://klx.co.in/webservice.php?action=ApproveFranchiseeWithdrawalRequest",  $( "#frmapprove" ).serialize(),function( data ) {
        ...
        if (obj.status=="failure") {
            ...
            html = "<div style='width:100%;padding:10px;background:#fff;text-align:center;color:#222'><img src='https://klx.co.in/klxadmin/assets/tick.jpg' style='width:128px'><br><br>Approved<br></div>";
            html += "<br>" + buttons;
...
function DoReject(RequestID) {
    var buttons = "<div style='text-align:center;padding:10px;'><button type='button' class='btn btn-secondary btn-sm' onclick='location.href=\"dashboard.php\"'>Home</button>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='location.href=\"dashboard.php?action=Requests/FranchiseeWithdrawalRequests&filter=Rejected\"'>Continue</button></div>";
    $("#confrimation_text").html(loading);
    
    $.post( "https://klx.co.in/webservice.php?action=RejectFranchiseeWithdrawalRequest",  $( "#frmapprove" ).serialize(),function( data ) {
        var obj = JSON.parse(data); 
        var html = "";
        if (obj.status=="failure") {
            html = "<div style='width:100%;padding:10px;background:#fff;text-align:center;color:red'><img src='https://klx.co.in/klxadmin/assets/accessdenied.png' style='width:128px'><br><br>Transaction failed.<br>"+obj.message;
            ...
            html = "<div style='width:100%;padding:10px;background:#fff;text-align:center;color:#222'><img src='https://klx.co.in/klxadmin/assets/tick.jpg' style='width:128px'><br><br>Rejected<br></div>";
            html += "<br>" + buttons;
        }
        $("#confrimation_text").html(html);
        $('#ConfirmationPopup').modal("show");
