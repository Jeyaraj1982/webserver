<?php
    ...
    if ($_SESSION['USER']['ismobileverified']==1) {
    ...
    if (!(isset($_SESSION['USER']['otp']))) {
        ...
        MobileSMS::sendSMS($_SESSION['USER']['mobile'],"Your klx.co.in verification Code is ".$code,$_SESSION['USER']['userid']);     
    }
    
    if(isset($_POST['btnsubmit2'])) {
        $resendcount = $mysql->select("select * from `_tbl_Log_MobileSMS` where `SmsTo`='".$_SESSION['USER']['mobile']."' and date(`MessagedOn`)=date('".date("Y-m-d H:i:s")."')"); 
        
        if(sizeof($resendcount)>5){
            echo $obj->printError("You have reached resend limits"); 
        } else {
            $code = rand(11111,99999);
            $_SESSION['USER']['otp']=$code;
            MobileSMS::sendSMS($_SESSION['USER']['mobile'],"Your klx.co.in verification code is ".$code,$_SESSION['USER']['userid']);    
    ...
    if (isset($_POST['loginbtn'])) {
        if ($_POST['verificationcode']==$_SESSION['USER']['otp']) {
            $mysql->execute("update `_jusertable` set `ismobileverified`='1',`mobileverifiedon`='".date("Y-m-d H:i:s")."' where `userid`='".$_SESSION['USER']['userid']."'");
            $record = $mysql->select("select * from `_jusertable` where  `userid`='".$_SESSION['USER']['userid']."'");
            $_SESSION['USER'] = $record[0];
            echo " <script>location.href='mypage.php?vm=1';</script>";
            exit;
        } else {
...
<link rel="stylesheet" href="https://www.klx.co.in/assets/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://www.klx.co.in/assets/css/atlantis.css">
...
<body class="login">
    <div class="wrapper wrapper-login">
        ...
        <h3 class="text-center">
                <img src="https://www.klx.co.in/assets/cms/klx_log.png" alt="klx" class="navbar-brand" style="height:65px;">
                <br>Mobile Verification</h3>
            
            <?php $resendcount = $mysql->select("select * from _tbl_Log_MobileSMS where SmsTo='".$_SESSION['USER']['mobile']."' and date(MessagedOn)=date('".date("Y-m-d H:i:s")."')"); 
                  if(sizeof($resendcount)<5){ ?>
                  <form action="" method="POST" >
                    <p align="center">we sent a verfication code your mobile number <?php if($_SESSION['USER']['countryid']==1){ echo "+91-";}?> <?php echo $_SESSION['USER']['mobile'];?><br><br></p>
                        <div class="login-form">
                            <div class="form-group form-floating-label">
                                <input id="username" name="verificationcode" value="<?php echo $_POST['verificationcode'];?>" type="text" class="form-control input-border-bottom" required>
                                <label for="username" class="placeholder">Verification Code</label>
                            </div>
                            <div class="form-action mb-3">
                                <button type="submit"  name="loginbtn" id="loginbtn" class="btn btn-primary btn-rounded btn-login" style="width:auto;padding:4px 20px;">Verify Code</button>
                                <br>
                                <div id="resend" style="padding: 10px;padding-top: 20px;">
                                
                                </div>
                               
                            </div>  
                            <div class="form-action mb-3">
                                 <a href="https://www.klx.co.in/logout" style="color:#555">Logout</a>
                            </div>
                    </form>  
                  <?php } else { ?>
                            <div class="form-action mb-3">
                                <img src="https://www.klx.co.in/assets/img/report.png">&nbsp;<?php echo "You have reached resend limits";?><br><br>
                                <a href="https://www.klx.co.in/logout" style="color:#555">Logout</a>
                            </div>
                  <?php } ?>
                        </div>
            ...
            <form action="" method="post">
                <input type="hidden" value="<?php echo $_SESSION['USER']['mobile'];?>">
                <button type="submit" hidden="hidden" name="btnsubmit" id="btnsubmit2" class="btn btn-primary btn-rounded btn-login">Continue</button>
            </form>
        </div>
    
 
    <script src="https://www.klx.co.in/assets/js/jquery.3.2.1.min.js"></script>
    <script src="https://www.klx.co.in/assets/js/jquery-ui.min.js"></script>
    <script src="https://www.klx.co.in/assets/js/popper.min.js"></script>
    <script src="https://www.klx.co.in/assets/js/bootstrap.min.js"></script>
    <script src="https://www.klx.co.in/assets/js/atlantis.min.js"></script>
     ...
     var tmr =    setInterval(function(){
            $('#resend').html("resend code will enable with in "+i+" sec");
            if (i<0) {
              $('#resend').html('<a href="javascript:void(0)" onclick="Resendverificationcode()">Resend</a>');  
              clearInterval(tmr);
            }
            i--;
        },1000);
