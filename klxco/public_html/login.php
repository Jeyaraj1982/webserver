     ...
     include_once("config.php"); 
       $obj = new CommonController();
         if (isset($_POST['loginbtn'])) {
          $record = $mysql->select("select * from _jusertable where (email='".trim($_POST['email'])."' or  mobile='".trim($_POST['email'])."') and pwd='".trim($_POST['upass'])."'");
          if (sizeof($record)>0) {
            $_SESSION['USER'] = $record[0];
            echo " <script>location.href='".country_url."mypage';</script>";
          } else {
              echo $obj->printError("Invalid Login Details");
          }
    }       
 ?>   
<!DOCTYPE html>
           ...
           <a href="<?php echo country_url;?>" class="close-quick-sidebar" style="float: right;font-size:20px;margin-top:-40px;color:#ccc">
            ...
            <div class="login-form">
                <div class="form-group form-floating-label">
                    <input id="username" name="email" value="<?php echo $_POST['email'];?>" type="text" class="form-control input-border-bottom" required>
                    <label for="username" class="placeholder">Email / MobileNumber</label>
                </div>
                <div class="form-group form-floating-label">
                    <input id="password" name="upass" type="password" value="<?php echo $_POST['upass'];?>" class="form-control input-border-bottom" required>
               ...
               <!-- <div class="row form-sub m-0">
                ...
                <div class="form-action mb-3">
                    <button type="submit"  name="loginbtn" class="btn btn-primary btn-rounded btn-login">Sign In</button>
                </div>
                <div class="login-account">
                    <a href="<?php echo path_url;?>forgetpassword" id="show-signup" class="link">Forget Password</a><br>
                    <span class="msg">Don't have an account yet ?</span>
                    <a href="<?php echo path_url;?>register" id="show-signup" class="link">Sign Up</a>
                </div>
            </div>
            </form>
        </div>
