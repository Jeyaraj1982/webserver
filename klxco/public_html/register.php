<?php
     ...
     include_once("config.php"); 
       $obj = new CommonController(); 
        ...
        if (sizeof($mysql->select("select * from _jusertable where upper(email)='".strtoupper(trim($_POST['email']))."'"))>0) {
        ...
        if (strlen(trim($_POST['pwd']))>=8) {
            ...
            echo $obj->printError("Password requires minimum 8 Characters");
        }
        
        if ($_POST['agree']!="on") {
            
            echo $obj->printError("Please agree terms and condition");
        }
        
            if ($obj->isEmptyField($_POST['personname'])) {
            echo $obj->printError("Name Shouldn't be blank");
            }
            if ($obj->isEmptyField($_POST['mobile'])) {
            //echo $obj->printError("Mobile Number Shouldn't be blank");
            } 
            if ($obj->isEmptyField($_POST['email'])) {
            //echo $obj->printError("Email Address Shouldn't be blank");
            } 
            if ($obj->isEmptyField($_POST['dob'])) {
            //echo $obj->printError("Date Of Birth Shouldn't be blank");
            } 
            if ($obj->isEmptyField($_POST['pwd'])) {
            echo $obj->printError("Password Shouldn't be blank");
            } 
            if ($obj->isEmptyField($_POST['state'])) {
            //echo $obj->printError("State Name Shouldn't be blank");
            } 
            
            if ($obj->isEmptyField($_POST['district'])) {
           ...
           if (JUsertable::addUser($param)>0) {
               ...
               echo "<script>location.href='".country_url."accountcreated';</script>";
              $obj->printSuccess("Register added successfully") ;
              unset($_POST)  ;
           } else {
                $obj->printError("Error adding Registers");   
           }
...
<body class="login">
    <div class="wrapper wrapper-login">
        ...
        <div class="container container-signup animated fadeIn">
         <a href="<?php echo country_url;?>" class="close-quick-sidebar" style="float: right;font-size:20px;margin-top:-40px;color:#ccc">
            ...
            <div class="login-form">
                <div class="form-group form-floating-label">
                    <input  id="fullname" name="personname" type="text"  value="<?php echo $_POST['personname'];?>"  class="form-control input-border-bottom" required>
                ...
                <div class="row form-sub m-0">
                    <div class="custom-control custom-checkbox">
                        ...
                        <label class="custom-control-label" for="agree">I agree <a href="https://www.klx.co.in/Terms">terms & conditions.</a></label>
                    </div>
                </div>
                <div class="form-action">
                    <button type="submit" name="save" class="btn btn-primary btn-rounded btn-login">Sign Up</button>
                </div>
                 <div class="login-account">
                 <br>
                    <span class="msg">I have an account?</span>
                    <a href="<?php echo path_url;?>login" id="show-signup" class="link">Sign In</a>
                </div>
            </div>
            </form>
        </div>
