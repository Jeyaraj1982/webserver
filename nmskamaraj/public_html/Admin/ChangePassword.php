...
include_once("LeftMenu.php"); 
    if (isset($_POST['btnsubmit'])) {
        $data = $mysql->select("select * from `admintable` where `AdminID`='".$_SESSION['User']['AdminID']."'");
        if($data[0]['Password']==$_POST['CurrentPassword']) {
            $mysql->execute("update `admintable` set `Password`='".$_POST['confirmnewpassword']."' where `AdminID`='".$_SESSION['User']['AdminID']."'"); 
            unset($_POST);
