<?php 
  ...
  $Members = $mysql->select("SELECT * FROM _tbl_Members 
                               ...
                               ON _tbl_Members.CurrentPackageID=_tbl_Settings_Packages.PackageID where Date(`_tbl_Members`.`CreatedOn`)>=Date('".$_POST['From']."') and Date(`_tbl_Members`.`CreatedOn`)<=Date('".$_POST['To']."')  and  `_tbl_Members`.`MemberCode` in (SELECT `ChildCode` FROM  `_tblPlacements` WHERE `PlacedByCode`='".$_GET['MCode']."'  ORDER BY `PlacementID` DESC)");
?>
