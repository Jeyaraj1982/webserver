 <?php 
    if($_GET['status']=="All"){ 
        $products = $mysql->select("select * from _tbl_products where md5(BrandID)='".$_GET['id']."' order by ProductID Desc");
        $title="All";
    }if($_GET['status']=="Active"){
        $products = $mysql->select("select * from _tbl_products where md5(BrandID)='".$_GET['id']."' and IsActive='1' order by ProductID Desc");
        $title="Active";
    }if($_GET['status']=="Disable"){
        $products = $mysql->select("select * from _tbl_products where md5(BrandID)='".$_GET['id']."' and IsActive='0' order by ProductID Desc");
        $title="Disable";
    }
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="card-title">
                                        Manage Products
                                    <br>
                                    <span style="font-size: 15px"><?php echo $data[0]['BrandName']."-".$title;?></span>  </div>
                                </div>
                                <div class="col-md-8" style="text-align: right;">
                                    <a href="dashboard.php?action=Brands/viewproducts&fr=br&id=<?php echo $_GET['id'];?>&status=All" <?php if($_GET['status']=="All"){ ?> style="font-size:bold;text-decoration:underline;"<?php } ?>>All Products</a>&nbsp;|&nbsp;
                                    <a href="dashboard.php?action=Brands/viewproducts&fr=br&id=<?php echo $_GET['id'];?>&status=Active" <?php if($_GET['status']=="Active"){ ?> style="font-size:bold;text-decoration:underline;"<?php } ?>>Active Products</a>&nbsp;|&nbsp;
                                    <a href="dashboard.php?action=Brands/viewproducts&fr=br&id=<?php echo $_GET['id'];?>&status=Disable" <?php if($_GET['status']=="Disable"){ ?> style="font-size:bold;text-decoration:underline;"<?php } ?>>Disable Products</a>
                                </div>
                            </div>
                        </div>                                       
                        <div class="card-body">                                                                                                                                             
