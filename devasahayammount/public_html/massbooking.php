...
<?php
if (isset($_POST['adatepicker_f'])) {
   $_POST['adatepicker_b'] = $_POST['adatepicker_f'];
}
    if (isset($_POST['paynow'])) {
        ...
        if ( strtotime(date("Y-m-d")) > strtotime($_POST['adatepicker_b']))  {
            $errc++;
            $err[]="Enter select valid date";
            $date = "Please select valid date . <span style='font-size:10px;'>(தயவு செய்து சரியான  தேதியை தெரிவு செய்யவும்    )</span>";
        }
       ...
       if ($errc==0) {
            ...
            $id = $mysql->insert("_massbooking",array("dateofpreferred" => $_POST['adatepicker_b'],
                                                      ...
                                                      "timing"          => $_POST['masstime'],
                                                      "paymentstatus"   => "REQUESTING"));

            $amount        = $_POST['amount'];  
            $accountid     = "16185";
            $return_url    = "http://www.devasahayammountshrine.com/epayment.php?key=dsmount&req=mb&rand={DR}";
            $mode          = "LIVE";
...
?>
    <form  method="post" action="https://secure.ebs.in/pg/ma/sale/pay" name="frmTransaction" id="frmTransaction" target="_self">
        ...
        <input name="name" type="hidden" maxlength="255" value="<?php echo strtoupper(trim($_POST['clientname']));?>" />
        ...
        <input name="ship_name" type="hidden" maxlength="255" value="<?php echo strtoupper(trim($_POST['clientname']));?>" />
        <input name="ship_address" type="hidden" maxlength="255" value="<?php echo $addressline2;?>" />
...
<table cellpadding="5" cellspacing="0" style="border:0px">
    <tr>
        <td style="border:none;padding:0px"><span style="color:#222">Book for</span></td>
        <td ><select name="bookfor" style="border:1px solid #ccc !important;margin:0px;line-height:1;padding:10px 10px;">
        <option value="Giving Thanks">Giving Thanks (நன்றி )</option>
                <option value="Giving Thanks">இறைஊழியர் </option>
        <option value="RIP">RIP</option>
        <option value="Birthday Celebration">வேண்டுதல் </option>
        <option value="Birthday Celebration">Birthday Celebration (பிறந்த நாள் )</option>
        <option value="Marriage Celebration">Marriage Celebration (திருமண நாள் )</option>
        <option value="Giving Thanks">Others/இதர </option>
        </select></td>
    </tr>
    <tr>
        <td style="border:none;padding:0px" ><span style="color:#222">Intention</span> </td>
        <td><textarea name="intention" style="border:1px solid #ccc !important;margin:0px;line-height:1;padding:10px 10px;"><?php echo $_POST['intention'];?></textarea>
            <?php if (isset($intention)) {echo "<br><span style='color:red'>".$intention."</span>"; } ?>
        </td>
    </tr>                        
    <tr>
        <td style="border:none;padding:0px"><span style="color:#222">Donor's Name</span> </td>
        <td><input type="text" name="clientname" style="border:1px solid #ccc !important;margin:0px;line-height:1;padding:10px 10px;" maxlength="15" value="<?php echo $_POST['clientname'];?>">
         <?php if (isset($donorname)) {echo "<br><span style='color:red'>".$donorname."</span>"; } ?>
        </td>
    </tr>    
    <tr style="height:50px;">
        <td style="border:none;padding:0px"><span style="color:#222">Date of Preferred</span>  
        </span></td>
        <td>
        <table cellpadding="0" cellspacing="0">
            <tr>
                <td>
                    <input type="text" readonly="readonly" name="adatepicker_b" id="adatepicker_b" value="<?php echo (isset($_POST['adatepicker_b'])) ? $_POST['adatepicker_b'] : date("Y-m-d");?>" style="width:80px;text-align:center;">
                </td>
                <td>&nbsp;&nbsp;</td>
                <td>
                    <div id="m_adatepicker_b" style="float: left;"></div>
                </td>
            </tr>
            <tr>
                <td colspan="2"><?php if (isset($date)) {echo "&nbsp;&nbsp;<span style='color:red;clear:both'>".$date."</span>"; } ?></td>
            </tr>
        </table>
          
          <script>new JsDatePick({limitFromToday: true,useMode:2,target:"adatepicker_b",dateFormat:"%Y-%m-%d",yearsRange:[2015,2020]});</script> 
          
        </td>
    </tr> 
    <tr>
        <td style="border:none;padding:0px"><span style="color:#222">Donation Amount</span></td>
        <td><input type="text" name="amount" style="border:1px solid #ccc !important;margin:0px;line-height:1;padding:10px 10px;"  maxlength="10" value="<?php echo $_POST['amount'];?>"> <span style="font-size:11px;">&nbsp;&nbsp;(Min: Rs. 90 Maximum: As your choice)</span>
    ...
    <tr>
        <td style="border:none;padding:0px"><span style="color:#222">Address</span>
        </td>
        <td><input type="text" name="addressline1" style="border:1px solid #ccc !important;margin:0px;line-height:1;padding:10px 10px;" maxlength="50" value="<?php echo $_POST['addressline1'];?>">
        <?php if (isset($addressline1)) {echo "<br><span style='color:red'>".$addressline1."</span>"; } ?>
        </td>
    </tr>   
     
    <tr>
        <td style="border:none;padding:0px"><span style="color:#222">Pin Code</span> 
        </td>
        <td><input type="text" name="pincode" style="border:1px solid #ccc !important;margin:0px;line-height:1;padding:10px 10px;" maxlength="6" value="<?php echo $_POST['pincode'];?>">
        <?php if (isset($pincode)) {echo "<br><span style='color:red'>".$pincode."</span>"; } ?>
        </td>
    </tr>
    <tr>
        <td style="border:none;padding:0px"><span style="color:#222">Email ID</span> 
        </td>
        <td><input type="text" name="emailid" style="border:1px solid #ccc !important;margin:0px;line-height:1;padding:10px 10px;"   value="<?php echo $_POST['emailid'];?>">
        <?php if (isset($emailid)) {echo "<br><span style='color:red'>".$emailid."</span>"; } ?>
        </td>
    </tr>
    <tr>
        <td style="border:none;padding:0px"><span style="color:#222">Mobile Number</span> 
        </td>
        <td><input type="text" name="mobilno" style="border:1px solid #ccc !important;margin:0px;line-height:1;padding:10px 10px;" maxlength="10" value="<?php echo $_POST['mobilno'];?>">
    ...
    <tr>
        <td style="border:none;padding:0px"><input type="submit" value="Book Now" class="myButton" name="paynow"></td>
    </tr>
</table> 
</form>   
<br><br>
...
<script>
<?php if (isset($_POST['adatepicker_b'])) { ?>
    getMasses('<?php echo $_POST['adatepicker_b'];?>','adatepicker_b');
<?php } else { ?>
getMasses('<?php echo date("Y-m-d");?>','adatepicker_b');
<?php } ?>
</script> 
