<?php
$data=$mysql->select("select * from _tbl_sliders where md5(SliderID)='".$_GET['id']."'");
    if (isset($_POST['btnsubmit'])) {
        $ErrorCount =0;
            
            $dupSlider = $mysql->select("select * from _tbl_sliders where SliderImage='".$_FILES["uploadimage1"]["name"]."' and SliderID<>'".$data[0]['SliderImage']."'");
           ...
           <?php }
            ...
            if($ErrorCount==0){
                if(strlen($_FILES["uploadimage1"]["name"])>0) {                                                 
                 ...
                 $mysql->execute("update _tbl_sliders set `SliderImage` ='".$file."',
                        ...
                        <form id="exampleValidation" method="POST" action="" onsubmit="return SubmitSlider();" enctype="multipart/form-data">
                            <div class="card-body">
                                ...
                                <div class="form-group form-show-validation row">
                                    ...
                                    <?php $Slids = $mysql->select("select * from _tbl_sliders");?>
                            ...
                            <div class="card-action">
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <a href="dashboard.php?action=Slider/list" class="btn btn-warning btn-border">Back</a>
                                    </div>                                        
                                </div>
                            </div>
                        </form>
