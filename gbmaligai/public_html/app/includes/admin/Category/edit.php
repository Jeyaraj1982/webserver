                    ...
                    <div class="card">
                        ...
                        <form id="exampleValidation" method="POST" action="" onsubmit="return SubmitCategory();" id="addProduct" enctype="multipart/form-data">
                            <div class="card-body">
                                ...
                                <?php
                                    $Categories = $mysql->select("select * from _tbl_category   ")
                                ?>
                                <div class="form-group form-show-validation row">
                                    <label for="name">Display Order</label>
                                    <select class="form-control" name="ListOrder" id="ListOrder">
                                    <?php for($i=1;$i<=sizeof($Categories);$i++) {?>
                                        <option value="<?php echo $i;?>" <?php echo (isset($_POST[ 'ListOrder'])) ? (($_POST[ 'ListOrder']==$i) ? " selected='selected' " : "") : (($data[0]['ListOrder']==$i) ? " selected='selected' " : "");?>><?php echo $i;?></option>
                                    <?php } ?>
