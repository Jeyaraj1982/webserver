...
for($index = 0;$index < $countfiles;$index++){
    ...
    if(in_array($ext, $valid_ext)){
        ...
        //if($_FILES['file']['size'] <= 100970000) { //10 MB (size is also in bytes)
        if(true) { //10 MB (size is also in bytes)
            if (move_uploaded_file($_FILES['files']['tmp_name'][$index],$path)){
              
             $exten = explode(".",strtolower($path));
$stamp = imagecreatefrompng('script/klx_75.png');             
...
//imagejpeg($im, $save_watermark_photo_address, 80)
                ...
                $id = $mysql->insert("_tbl_product_additional_image",array("ProductID"          => $_REQUEST['ProductID'],
                                                                           "ProductImage"       => $nfilename,
                                                                           "AddedOn"            => date("Y-m-d H:i:s")));
                    
                
                $temp = array("error"=>"0","filename"=>$nfilename,"ProductImageID"=>$id);
            } else {
