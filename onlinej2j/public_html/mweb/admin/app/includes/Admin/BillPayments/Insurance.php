<?php
    ...
    if (isset($_GET['filter']) && $_GET['filter']=="paid") {
        $Members = $mysql->select("select txn.*,mem.MobileNumber from _tbl_transactions as txn left join _tbl_member as mem on mem.MemberID=txn.memberid  where txn.operatorcode in (".$OperatorTypeCodes.") and txn.TxnStatus='success' order by txn.txnid desc");
        $title .= " Success Transactions";
    } if (isset($_GET['filter']) && $_GET['filter']=="reversed") {
        $Members = $mysql->select("select txn.*,mem.MobileNumber from _tbl_transactions as txn left join _tbl_member as mem on mem.MemberID=txn.memberid  where txn.operatorcode in (".$OperatorTypeCodes.") and txn.TxnStatus='reversed' order by txn.txnid desc");
        $title .= " Reversed Transactions";
    } if (isset($_GET['filter']) && $_GET['filter']=="requested") {
        $Members = $mysql->select("select txn.*,mem.MobileNumber from _tbl_transactions as txn left join _tbl_member as mem on mem.MemberID=txn.memberid   where txn.operatorcode in (".$OperatorTypeCodes.") and txn.TxnStatus='requested' order by txn.txnid desc");
...
<div style="padding:25px">
    ...
    <div class="row" style="margin-bottom:10px;">
        <div class="col-md-12" style="text-align: right;">
            <a href="dashboard.php?action=BillPayments/Insurance&filter=requested"><small <?php if($_GET['filter']=="requested") { ?> style="text-decoration: underline;font-weight:bold;" <?php } ?>>Requested</small></a>&nbsp;|&nbsp; 
            <a href="dashboard.php?action=BillPayments/Insurance&filter=paid" ><small <?php if($_GET['filter']=="paid") { ?> style="text-decoration: underline;font-weight:bold;" <?php } ?>>Paid</small></a>&nbsp;|&nbsp;
            <a href="dashboard.php?action=BillPayments/Insurance&filter=reversed"><small <?php if($_GET['filter']=="reversed") { ?> style="text-decoration: underline;font-weight:bold;" <?php } ?>>Reversed</small></a>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
                         ...
                         <table id="basic-datatables" class="display table table-striped table-hover" >
                            <thead>        
                                <tr>
                                    ...
                                    <th class="border-top-0"><b>DOB</b></th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $Member['rcnumber'];?></td>
                                    ...
                                    <td style="text-align: right;"><a href="javascript:openTxnInsuranceViewDialog('<?php echo $Member['txnid'];?>')">View</a></td>
                                    <?php } elseif (isset($_GET['filter']) && $_GET['filter']=="reversed") { ?>
                                    <td style="text-align: right;"><?php echo $Member['TxnStatus'];?></td>
                                    ...
                                    <td style="text-align: right;">
                                        <a href="javascript:openTxnInsuranceViewDialog('<?php echo $Member['txnid'];?>')">View</a><br>
...
<div class="modal fade" id="viewRxn" tabindex="-1" role="dialog" aria-labelledby="viewRxn" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content" id="txnData" style="overflow: auto;">
...
<script>
    function openTxnInsuranceViewDialog(txnid) {                                  
       $('#txnData').html("Loading ...."); 
         $('#viewRxn').modal("show");
        $.ajax({url:'webservice.php?action=getInsuranceTxn&txn='+txnid,success:function(data){
