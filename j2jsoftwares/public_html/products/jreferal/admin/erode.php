<h2 style="text-align: left;font-family: arial;">Erode EarnMoneyTech Amdin Area </h2>     
<div style="border-bottom:1px solid #D4E3F6;"></div><br>  
    <?php
////Erode         
               $registeredClients = $mysql->select("select * from _clients where lcase(streetname) like '%erode%' order by id desc");
        $blockedClients = $mysql->select("select * from _clients where  lcase(streetname) like '%erode%' and isblock=1   order by id desc");
        $newClients = $mysql->select("select * from _clients where  lcase(streetname) like '%erode%' and isblock=-1 and isactive=0 order by id desc");   
        $activeClients = $mysql->select("select * from _clients where  lcase(streetname) like '%erode%' and isblock=0 and isactive=1 order by id desc");   
