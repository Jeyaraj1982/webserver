<h2 style="text-align: left;font-family: arial;">Blocked Clients</h2>  
<?php // $data = $mysql->select("select * from _clients where isblock=1  order by id desc"); ?>

<?php

if (isset($_POST['distName'])) {
    $distName = strtolower(trim(trim(str_replace("\n","",$_POST['distName']))));
     $data = $mysql->select("select * from _clients where LOWER(trim(streetname)) like '%".$distName."%' isblock=1  order by id desc"); 
 
} else {
     if ($_SESSION['country']=="Malaysia") {      
         $data = $mysql->select("select * from _clients where isblock=1 and country='malaysia' order by id desc"); 
     } else {
 $data = $mysql->select("select * from _clients where isblock=1  order by id desc");         
     }
 
} 

if ( $_SESSION['user']['role']=="erode" ) {
    $data = $mysql->select("select * from _clients where  lcase(streetname) like '%erode%' and isblock=1  order by id desc");         
}
