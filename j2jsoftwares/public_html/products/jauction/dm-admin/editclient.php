...
<form action="updateclient.php" method="post">
    ...
    <div style="border-bottom:1px solid #D4E3F6;"></div><br>
        ...
        <table cellpadding='5' cellspacing='5' style='font-size:12px;' width="100%">
            ...
            <tr><td>Userlink</td><td>:</td><td>http://www.dealmaass.in/u/<?php echo $data[0]['userlink']; ?></td></tr>
            <tr><td>Activation On</td><td>:</td><td><?php echo $data[0]['activatedon']; ?></td></tr> 
            <tr><td>Visitors</td><td>:</td><td>
            <?php
...
?>
    ...
    $te = $mysql->select("select * from _visitor where userid='3002649'");
    foreach($te as $t) {
        $tc = $mysql->select("select * from _clients_account where particulars = 'Earn via visting your link - ".$t['id']);
        if (sizeof($tc)==1) {
        } else {
            $tss = $mysql->execute("delete from _clients_account where particulars = 'Earn via visting your link - ".$t['id']);              
        }
    }
    ...
    if ($_SESSION['user']['role']=="admin") {
     ...
     //   $sql= "delete from _visitor where id in (";
        $c = 0;
        foreach($forid as $f) {
           $d=$mysql->execute("delete from _visitor where id=".$f);
           $dd=$mysql->execute("delete from _clients_account where particulars = 'Earn via visting your link - ".$f."'");
       ...
       echo  "<div style='border:1px solid #e5e5e5;margin:10px;padding;10px;color:red;font-weight:bold;font-family:arial;font-size:12px;'>".$c." records deleted successfully</div>";
     ...
     <?php  if ($_SESSION['user']['role']=="admin") {  ?>
        ...
        <?php
            if (isset($_POST['addComment'])) {
                  $ins = array("clientid"=>$_POST['clientid'],
                  "reason"=>$_POST['reason'],
                  "status"=>$_POST['status'],
                  "postedon"=>date("Y-m-d H:i:s"),
                  "postedby"=> $_SESSION['user']['role']
                  
                  );
                  $mysql->insert("_comments",$ins);
