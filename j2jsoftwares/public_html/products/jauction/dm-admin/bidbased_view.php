...
<div style="float:left;width:370px;height:415px;border:1px solid #cccccc;margin-right:<?php echo ($c==0) ? '18px' : '0px'; ?>;margin-bottom: <?php echo ($c==0) ? '18px' : '0px'; ?>;" >
    <table width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td colspan="2" style="background:#f1f1f1;padding:5px;height:40px;border-top:1px solid #aaaaaa;font-family:arial;font-size:12px;">Current Lowest Unique bidder:   <?php
...
$bid = $dealmaass->getLowestBidRate($item['itemid']);
    foreach($mysql->select("select userid, count(*) as c, bidrate from  _bids where productid='".$item['itemid']."' group by bidrate") as $d) {
        
        ?>
            <tr style="<?php echo ($bid[0]['bidrate']==$d['bidrate']) ? 'background:lime' : '';?>">
                <td><?php echo  $d['bidrate'];?></td>
                <td><?php echo  $d['c'];?></td>
            </tr>
        <?php
    }
