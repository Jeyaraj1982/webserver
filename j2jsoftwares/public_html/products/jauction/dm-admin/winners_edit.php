...
<?php
 include_once("config.php");   
       ...
       if (isset($_POST['isDelete'])) {
           $mysql->execute("delete from _tblwinners where md5(id)='".$_REQUEST['item']."'");
           ?>
           <script>window.open('winners.php','rightW');</script>
    ...
    if (isset($_POST['isSubmit'])) {
        ...
        $sql = "update _tblwinners set productname='".$_POST['productname']."', testimonials='".$_POST['description']."', test_name='".strtoupper($_POST['itemname'])."' ";
        
        if ((move_uploaded_file($_FILES["file"]["tmp_name"],"../couriers/".$filename)) ) {
            
            $sql .= ", photopath='".$filename."' ";  
        }
        
        if ((move_uploaded_file($_FILES["file1"]["tmp_name"],"../couriers/".$filename1)) ) {
            
            $sql .= ", courierpath='".$filename1."' ";  
        ...
        $item =  $mysql->select("select * from _tblwinners where md5(id)='".$_REQUEST['item']."'");
    $item = $item[0];
?>
...
<form action="winners_edit.php?item=<?php echo $_REQUEST['item']; ?>" method="post"   enctype="multipart/form-data">
    <table  style="font-size:12px;font-family:arial;" cellpadding="5" cellspacing="0">
        ...
        <tr>
            <td>Product Name</td>
            <td><input type="text" name="productname" value="<?php echo $item['productname'];?>"></td>
        </tr>
         
        <tr>
            <td>Product Image</td>
            <td>
            <img src="../couriers/<?php echo $item['photopath']; ?>" height="200"><br>
            <input type="file" name="file" ></td>
        </tr>
        <tr>
            <td>Courier Copy</td>
            <td>
            <img src="../couriers/<?php echo $item['courierpath']; ?>" height="200"><br>
        ...
        <tr>
            <td colspan="2"><input type="submit" value="Update Winner Courier" name="isSubmit">
            <input type="button"  onclick="window.open('winners.php','rightW');window.close('GoogleWindow')" value=" Close ">
