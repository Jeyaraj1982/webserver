...
<?php foreach($mysql->select("select * from _tbl_invoice order by invoiceid desc") as $invoices) { ?>
             <tr>
                <td style="border:1px solid #ccc;text-align: center;"><?php echo $invoices['invoiceid'];?></td>
                <td style="border:1px solid #ccc;text-align: center;"><?php echo $invoices['invoicedate'];?></td>
                <td style="border:1px solid #ccc;text-align: right;"><?php echo number_format($invoices['invoiceamount'],2);?></td>
                <td style="border:1px solid #ccc;text-align: center;"><?php echo ($invoices['paymentstatus']==0) ? "Success" : "Failed";?></td>
                <td style="border:1px solid #ccc;text-align: center;"><?php echo $invoices['transactionid'];?>&nbsp;</td>
                <td style="border:1px solid #ccc;">
                    <form action="viewInvoice.php" method="post" target="ts" onsubmit="window.open('viewInvoice.php','ts','height=500,width=550')">
