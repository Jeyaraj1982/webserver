<?php 
    ...
    if( $_GET['Status']=="All"){
        $sql= $mysql->select("select * from `_tbl_Members` where `IsMember`='1' and MapedTo='".$sql[0]['MemberID']."' order by `MemberID` desc ");
        $title="All Retailers";
    }
    if( $_GET['Status']=="Active"){
        $sql= $mysql->select("select * from `_tbl_Members` where `IsMember`='1'  and MapedTo='".$sql[0]['MemberID']."' and IsActive='1' order by `MemberID` desc ");
        $title="Active Retailers";
    }
    if( $_GET['Status']=="Inactive"){
        $sql= $mysql->select("select * from `_tbl_Members` where `IsMember`='1'  and MapedTo='".$sql[0]['MemberID']."' and IsActive='0' order by `MemberID` desc ");
        ...
        <div class="page-inner">
            <div class="form-group row">
                ...
                <div class="col-sm-6" style="text-align:right;padding-top:5px;color:skyblue;">
                    <a href="dashboard.php?action=Dealers/ManageRetailers&d=<?php echo $_GET['d'];?>&Status=All"><?php if($_GET['Status']=="All") { ?><small style="font-weight:bold;text-decoration:underline;"><?php } else{ ?><small><?php } ?>All</small></a>&nbsp;|&nbsp;
                    <a href="dashboard.php?action=Dealers/ManageRetailers&d=<?php echo $_GET['d'];?>&Status=Active"><?php if($_GET['Status']=="Active") { ?><small style="font-weight:bold;text-decoration:underline;"><?php } else{ ?><small><?php } ?>Active</small></a>&nbsp;|&nbsp;
                    <a href="dashboard.php?action=Dealers/ManageRetailers&d=<?php echo $_GET['d'];?>&Status=Inactive"><?php if($_GET['Status']=="Inactive") { ?><small style="font-weight:bold;text-decoration:underline;"><?php } else{ ?><small><?php } ?>Inactive</small></a>&nbsp;|&nbsp;
                </div> 
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header" style="padding-top:10px;padding-bottom:10px">
                            <div class="card-title">
                                <?php echo $title; ?>
                                <a href="dashboard.php?action=Dealers/CreateRetailer&d=<?php echo $_GET['d'];?>" style="float:right;color:#fff" class="btn btn-warning btn-sm">Create Retailer</a>
                            </div>
                        </div>
                        <div class="card-body">
