<?php
    ...
    if ($_SESSION['USER']['isemailverified']==1) {
    ...
    if (!(isset($_SESSION['USER']['otp']))) {
        ...
        $message = '<div style="padding:45px;background:#e5e5e5;margin:20px;border-radius:10px;padding-top:20px;">
        ...
        $mparam['Subject']="Rhole.in :) Email Verification";
        $mparam['Message']=$message;
        MailController::Send($mparam,$mailError);        
    }
    
    if(isset($_POST['btnsubmit'])) {
        $resendcount = $mysql->select("select * from `_tbl_logs_email` where `EmailTo`='".$_SESSION['USER']['email']."' and date(`EmailRequestedOn`)=date('".date("Y-m-d H:i:s")."')"); 
    ...
    if (isset($_POST['loginbtn'])) {
        if ($_POST['verificationcode']==$_SESSION['USER']['otp']) {
            $mysql->execute("update `_jusertable` set `isemailverified`='1',`emailverifiedon`='".date("Y-m-d H:i:s")."' where `userid`='".$_SESSION['USER']['userid']."'");
            $record = $mysql->select("select * from `_jusertable` where  `userid`='".$_SESSION['USER']['userid']."'");
            $_SESSION['USER'] = $record[0];
            echo "<script>location.href='mypage.php?em=1';</script>";
            exit;
        } else {
...
<head>
    ...
    <link rel="icon" href="<?php echo AppUrl;?>/assets/img/icon.ico" type="image/x-icon"/>
    <script src="<?php echo AppUrl;?>/assets/js/plugin/webfont/webfont.min.js"></script>
    <script>
    ...
    <link rel="stylesheet" href="<?php echo AppUrl;?>/assets/css/atlantis.css">
</head>
<body class="login">
    <div class="wrapper wrapper-login">
        ...
        <h3 class="text-center">
                ...
                <br><br>Email Verification</h3>
            <?php $resendcount = $mysql->select("select * from _tbl_logs_email where EmailTo='".$_SESSION['USER']['email']."' and date(EmailRequestedOn)=date('".date("Y-m-d H:i:s")."')"); 
                  if(sizeof($resendcount)<5){ ?>
                  <form action="" method="POST" >
                    <p align="center">we sent a verfication code your email <?php echo $_SESSION['USER']['email'];?><br><br></p>
                        <div class="login-form">
                            ...
                            <div class="form-action mb-3">
                                 <a href="<?php echo AppUrl;?>/logout" style="color:#555">Logout</a>
                            </div>
                    </form>  
                  <?php } else { ?>
                            <div class="form-action mb-3">
                                <img src="<?php echo AppUrl;?>/assets/img/report.png">&nbsp;<?php echo "You have reached resend limits";?><br><br>
                                <a href="<?php echo AppUrl;?>/logout" style="color:#555">Logout</a>
                            </div>
                  <?php } ?>
                        </div>
            ...
            <form action="" method="post">
                <input type="hidden" value="<?php echo $_SESSION['USER']['email'];?>">
                <button type="submit" hidden="hidden" name="btnsubmit" id="btnsubmit" class="btn btn-primary btn-rounded btn-login">Continue</button>
            </form>
        </div>
    
 
    <script src="<?php echo AppUrl;?>/assets/js/jquery.3.2.1.min.js"></script>
    <script src="<?php echo AppUrl;?>/assets/js/jquery-ui.min.js"></script>
    <script src="<?php echo AppUrl;?>/assets/js/popper.min.js"></script>
    <script src="<?php echo AppUrl;?>/assets/js/bootstrap.min.js"></script>
    <script src="<?php echo AppUrl;?>/assets/js/atlantis.min.js"></script>
    <script>
