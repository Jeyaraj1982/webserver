...
include_once("config.php");
    ...
    function getSubcategory($maincategory="") {
        ...
        if (sizeof($total_ads_incurrent_category)<4){
            ...
            if (sizeof($usr_packages)==0) {
                $category_info = "<script>$('#form_data').hide();document.getElementById('category_info').innerHTML=\"<div style='padding:10px;padding-left:0px;color:#4263eb;font-size:12px'>In this category, You have reached your free limits. You wait for expire posted ads or  <a href='".AppUrl."/in/upgrade/c0' class='btn btn-primary btn-sm' style='padding:3px 8px;float-right'>Upgrade Package</a></div>\";</script>";    
            } else {
                ...
                if (sizeof($usr_packages)==0) {
                    $category_info = "<script>$('#form_data').hide();document.getElementById('category_info').innerHTML=\"<div style='padding:10px;padding-left:0px;color:#4263eb;font-size:12px'>In this category, Your package(s) may be expired. You wait for expire posted  ads or  <a href='".AppUrl."/in/upgrade/c0' class='btn btn-primary btn-sm' style='padding:3px 8px;float-right'>Upgrade Package</a></div>\";</script>";    
                }   else {
                ...
                if (JFavorites::addFavorites($param)>0)  {
                    ...
                    if(sizeof($logview)==1){
                        ...
                        $message = '                                                                                  
                            ...
                            $mparam['Subject']="Rhole.in :) Your ad is Liked by ".$_SESSION['USER']['personname'];
    ...
    function getUpgradePackages() {
        ...
        ?>
        <form action="<?php echo AppUrl;?>/pay_upgrade_instamajo.php" method="post">
        ...
        if(isset($_POST["dosave"]) && $_POST['dosave']=="1") {
            ...
            if (sizeof($total_free_ads_incurrent_category)<4) {
                ...
                if (sizeof($availableads)>0) {
                    ...
                    return json_encode(array("status"=>"failure","message"=>'Your free limit has completed, Upgrade package and continue to post ads.<a href="'.AppUrl.'/in/upgrade/c0" class="btn btn-primary btn-sm">View Packages</a>'));
                }
            }
            }
            ...
            if ($obj->err==0) { 
                ...
                if($_SESSION['USER']['isstaff']==1){  
                    $adType="free";
                    MobileSMS::sendSMS($_POST['CustomerMobileNumber'],"Your Ad Posted on rhole.in Your rhole ad link ".path_url."v".$postedadid."-".$title." You can post more your ads this website, its totally free",$_SESSION['USER']['userid']); 
                }
