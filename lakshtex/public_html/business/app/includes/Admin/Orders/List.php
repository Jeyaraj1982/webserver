...
} elseif (isset($_GET['f']) && $_GET['f']=="new") {
    $Orders = $mysql->select("select * from `_tbl_Member_Orders` where OrderDelivered='0' order by `OrderID` desc"); 
} elseif (isset($_GET['f']) && $_GET['f']=="cash") {
    $Orders = $mysql->select("select * from `_tbl_Member_Orders` where PaymentStatus='Cash on delivery' order by `OrderID` desc"); 
}elseif (isset($_GET['f']) && $_GET['f']=="wallet") {
    $Orders = $mysql->select("select * from `_tbl_Member_Orders` where PaymentStatus='Wallet payment' order by `OrderID` desc"); 
}elseif (isset($_GET['f']) && $_GET['f']=="delivered") {
    $Orders = $mysql->select("select * from `_tbl_Member_Orders` where OrderDelivered='1' order by `OrderID` desc"); 
} else {
    $Orders = $mysql->select("select * from `_tbl_Member_Orders` order by `OrderID` desc"); 
}
?>
<div style="padding:25px">
    ...
    <div class="row" style="margin-bottom:10px;">
        <div class="col-md-12" style="text-align: right;">
            ...
            <a href="dashboard.php?action=Orders/List&f=new">New Orders</a>&nbsp;|&nbsp; 
            <a href="dashboard.php?action=Orders/List&f=cash">Cash On Delivery</a>&nbsp;|&nbsp; 
            <a href="dashboard.php?action=Orders/List&f=wallet">Paid By Wallet</a>&nbsp;|&nbsp; 
