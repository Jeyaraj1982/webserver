<?php 
    ...
    if(isset($_POST['viewTransaction'])){
        $earings = $mysql->select("select * from `_tbl_wallet_utility` where `MemberCode`='".$_SESSION['User']['MemberCode']."' and (date(TxnDate)>=date('".$fromDate."') and date(TxnDate)<=date('".$toDate."') ) order by AcTxnID Desc");
    }else{
        $earings = $mysql->select("select * from `_tbl_wallet_utility` where `MemberCode`='".$_SESSION['User']['MemberCode']."' order by AcTxnID Desc");
    }
?>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="card-title">
                                        Voucher History<br>
                                        <span style="font-size:12px;">Voucher Balance. <?php echo number_format(GetUtilityBalance($_SESSION['User']['MemberCode']),2);?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-striped table-bordered" id="basic-datatables" style="width: 100%;border-top:1px solid #ebedf2;border-bottom:1px solid #ebedf2;">
                                    ...
                                    <tr>
                                        ...
                                        <td style="text-align:right"><?php echo number_format($e['Balance'],2);?></td>
                                     </tr>
                                     <?php  }  ?>
