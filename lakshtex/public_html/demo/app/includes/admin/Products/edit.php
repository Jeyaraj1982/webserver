...
$data=$mysql->select("select * from _tbl_products where md5(ProductID)='".$_GET['id']."'");  
    if (isset($_POST['btnsubmit'])) {
        $ErrorCount =0;
            $dupemail = $mysql->select("select * from _tbl_products where ProductName='".$_POST['ProductName']."' and ProductID<>'".$data[0]['ProductID']."' and CategoryID='".$_POST['Category']."' and SubCategoryID='".$_POST['SubCategory']."'");
            ...
            if($ErrorCount==0){
               ...
               $SubCategory = $mysql->select("select * from _tbl_sub_category where SubCategoryID='".$_POST['SubCategory']."'");
                $AgeGroup = implode(",",$_POST["AgeGroup"]);
                $BrandSize = implode(",",$_POST["BrandSize"]);
                                   ...
                                   <input type="file" id='files' name="files[]" multiple style="display:none">
                                    ...
                                    <div class="card-body">
                                       ...
                                       <div class="form-group form-show-validation row">
                                            ...
                                            $AgeGroups = $mysql->select("select * from _tbl_size_age_group where IsActive='1'");
                                                
                                                   foreach($AgeGroups as $a){  
                                                      $check = "";  
                                                   if (in_array($a['Label'],$AgeGrps)) {
                                                       $check = "checked='checked'";
                                                   } 
                                            ?>
                                            <div class="col-3">
                                            <div class="custom-control custom-checkbox">
                                              <input type="checkbox" class="custom-control-input" id="<?php echo $a['AgeGroupID'];?>" value="<?php echo $a['Label'];?>" name="AgeGroup[]" <?php echo $check;?>>
                                                <label class="custom-control-label" for="<?php echo $a['AgeGroupID'];?>" style="font-weight: normal;"><?php echo $a['Label'];?></label>
                                            </div>
                                            </div>
                                            <?php } ?>
                                            </div>
                                       ...
                                       <div class="form-group form-show-validation row">
                                            ...
                                            <div class="col-sm-12">
                                            <?php 
                                                $sizes = explode(",", $data[0]['BrandSize']);
                                                $BrandSizes = $mysql->select("select * from _tbl_brand_size where IsActive='1'");
                                                
                                                   foreach($BrandSizes as $b){  
                                                      $check = "";  
                                                   if (in_array($b['Label'],$sizes)) {
                                                       $check = "checked='checked'";
                                                   }
                                            ?>
                                            
                                            <div class="custom-control custom-checkbox">
                                              <input type="checkbox" class="custom-control-input" id="BrndSize<?php echo $b['SizeID'];?>" value="<?php echo $b['Label'];?>" name="BrandSize[]" <?php echo $check;?>>
                                                <label class="custom-control-label" for="BrndSize<?php echo $b['SizeID'];?>" style="font-weight: normal;"><?php echo $b['Label'];?></label>
                                            </div>
                                            <?php } ?>
                                            </div>
                                            <span class="errorstring" id="ErrDetailDescription"><?php echo isset($ErrDetailDescription)? $ErrDetailDescription : "";?></span>
