    ...
    function putDate($date) {
     ...
     class SeqMaster {
        ...
        public static function GetNextMemberCode($MemberType=2) {
            ...
            $Rows   = $mysql->select("select count(*) as rCount from `_tbl_member`  WHERE `MemberCode` LIKE '1H%' ");
            ...
            $prefix = "1H";
            $count = 100002000030001;
            $Rows   = $mysql->select("select count(*) as rCount from `_tbl_member`  WHERE `MemberCode` LIKE 'IH%' ");
            return $prefix.($count+$Rows[0]['rCount']);
            }
        }
...
$mysql   = new MySql("localhost","glspost_user","mysqlPwd","glspost_database");

    $income = array(150,30,20,50,40,30,20,20,20,20,20,20);
    $income = array(150,30,20);                                                       
     ...
     function getTags($MemberCode,$TagNumber) {
      ...
      function findUplines($sponsorCode) {
         ...
         function getWithdrawableBalance($MemberCode) {
             global $mysql;
             $e = $mysql->select("select (sum(Credit)-Sum(Debit)) as bal from `_tbl_accounts` where `MemberCode`='".$MemberCode."'");
