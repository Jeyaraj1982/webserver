...
define("DbName","glspost_database");
define("DbUser","glspost_user");
define("DbPassword","mysqlPwd");
 
define("CreateMemberUsing","EPIN"); //EPIN / WALLET
...
$right_ids= $n_rightids;
     ...
     function getMemberCode($memberName) {
       ...
       // $d .= strtoupper(substr($memberName,0,3));
        ...
        return $member_code_prefix[0]['ParamValue'].str_pad((sizeof($count)+1),$member_code_length[0]['ParamValue'],"0",STR_PAD_LEFT);
       // return strlen(sizeof($count)+1).$member_code_length[0]['ParamValue'];
        // $d .= str_pad(strlen(sizeof($count)+1),$member_code_length[0]['ParamValue'],0,STR_PAD_LEFT);
...
function getIncome($level) {
    $income = array("0",30,10,8,16,48,28.8);
    return (isset($income[$level]))  ? $income[$level] : 0;
