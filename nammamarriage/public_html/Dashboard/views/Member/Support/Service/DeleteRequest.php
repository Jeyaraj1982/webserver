    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="post" action="" name="Deletefrm_<?php echo $_GET['Code'];?>" id="Deletefrm_<?php echo $_GET['Code'];?>">
                ...
                <div class="form-group row">
                    <div class="col-sm-4">
                        <a href="javascript:void(0)" onclick="showConfirmDeleteMember('<?php echo $_GET['Code'];?>')" class="btn btn-danger" name="Delete" style="font-family:roboto">Delete Member</a>
                    </div>
                </div>
            </form>
        </div>
...
<script>
function showConfirmDeleteMember(RequestForCode) {
      ...
      var content = '<div>'
                            ...
                            +'<div class="modal-footer">'  
                                ...
                                +'<button type="button" class="btn btn-primary" name="Delete" id="Delete"  onclick="SendOtpForDeleteMember(\''+RequestForCode+'\')" style="font-family:roboto">Yes</button>'
                            +'</div>'
                    +'</div>';
                $('#Delete_body').html(content);
}
function SendOtpForDeleteMember(formid) {
    var param = $("#Deletefrm_"+formid).serialize();
    $('#Delete_body').html(preloading_withText("Deleting profile ...","199"));
        $.post(getAppUrl() + "m=Member&a=SendOtpForDeleteMember",param,function(result) {
                 ...
                 var content = '<div id="otpfrm" >'
                                ...
                                + '<input type="hidden" value="'+formid+'" name="RequestForCode">'
                                    ...
                                    +'<div class="modal-body">'
                                            ...
                                            + '<div class="input-group">'
                                                + '<div class="col-sm-12">'
                                                    ...
                                                    + '<div class="col-sm-6">'
                                                        ...
                                                        + '<button type="button" onclick="DeleteMemberOTPVerification(\''+randString+'\')" class="btn btn-primary" name="btnVerify" id="verifybtn">Verify</button>'
                                                    + '</div>'
                                                    + '<div class="col-sm-3"></div>'
                                        ...
                                        + '</div>'
                                    + '</div>'
                                    + '<h5 style="text-align:center;color:#ada9a9">Did not receive the verification code?<a onclick="ResendOtpForDeleteMember(\''+randString+'\')" style="cursor: pointer;color: #1694b5;">&nbsp;Resend</a></h5>' 
                                + '</form>'
                             + '</div>';
                 $('#Delete_body').html(content);
        }
...
function DeleteMemberOTPVerification(frmId) {
        var param = $( "#"+frmId).serialize();
        $('#ChangeMobile_body').html(preloading_withText("Loading ...","195"));
        $.post( getAppUrl() + "m=Member&a=DeleteMemberOTPVerification",param).done(function(result) {
            if (!(isJson(result))) {
                $('#Delete_body').html(result);
                return ;
            }
