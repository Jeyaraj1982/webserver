<?php
    ...
    $backup_file = "/home/happylife2020/public_html/app/backup/sql/db_".date("Y_m_d_H_i_s"). '.sql';
    ...
    function writelog($text) {
        global $filename;
        $myfile = fopen("/home/happylife2020/public_html/app/backup/logs/payoutlogs/".$filename, "a") or die("Unable to open file!");
        fwrite($myfile, "\n[".date("Y-m-d H:i:s")."]\t".$text);
        fclose($myfile);
    }

    writelog("=== Binary Eligible Starts ===");
    $IsBinaryEligible = $mysql->select("select * from `_tbl_Members` Where Date(CreatedOn)<=Date('".$date."')");
    ...
    $IsPayoutEligible = $mysql->select("select * from `_tbl_Members` Where Date(CreatedOn)<=Date('".$date."')");
   ...
   foreach($IsPayoutEligible as $Member) {
       ...
       //$records = $mysql->select("select * from _tbl_payoutsummary where MemberCode='".$Member['MemberCode']."' and PayableAmount>0 ");
       ...
       if ($Member['IsPayoutEligible']==1 && sizeof($records)==0 && $totalLeftPV>0 && $totalRightPV>0)  { //First Payout 1:2 or 2:1
           ...
           if ($totalLeftPV>=$Member['PV'] && $totalRightPV>=$Member['PV'] ) {
               ...
               if ($totalLeftPV >= $Member['PV']) {
                   $debitLeft    = $Member['PV'];
                   $debitRight   = 0; //$Member['PV'];
                   $CompanyLeft  = $Member['PV'];
                   $CompanyRight = 0;
               } elseif ($totalRightPV >= $Member['PV']) {
                   $debitRight   = $Member['PV'];
                   $debitLeft    = 0; //$Member['PV'];
                   $CompanyLeft  = 0;
                   $CompanyRight = $Member['PV'];
               } elseif (($totalRightPV == $Member['PV']) && ($totalRightPV == $Member['PV'])) {
                   $debitRight   = 0;//$Member['PV'];
                   $debitLeft    = $Member['PV'];
                   $CompanyLeft  = 0;
                   $CompanyRight = $Member['PV'];
   ...
   if (sizeof($direct_referal)==0) {
       ...
       foreach($direct_referal as $dr) {
           ...
           if ($total_earning>0) {
               ...
               $charges = ($total_earning/10);
               $balance = $balance-$charges;
               $walletTxnID= $mysql->insert("_tbl_wallet_earnings",array("MemberID"         => $Member['MemberID'],
                                                                         "MemberCode"       => $Member['MemberCode'],
                                                                         "Particulars"      => "Charges 10% Direct Referal Payout process on ".$date,
                                                                         "TxnDate"          => date("Y-m-d H:i:s"),
                                                                         "Credits"          => '0',
                                                                         "Debits"           => $charges==0 ? '0' : $charges,
                                                                         "AvailableBalance" => $balance,
           ...
           writelog("<br>---  ---  Total Referal Earnings --- --- --- Rs.".$total_earning." --- --- Charges --- Rs.". ($total_earning*10/100). " --- --- Payable Amount --- --- Rs.". ($total_earning-($total_earning*10/100))."<br>"); 
       }
   }
     
   echo "<pre>";
        echo (file_get_contents("/home/happylife2020/public_html/app/backup/logs/payoutlogs/".$filename));
   echo "</pre>";
 ?>