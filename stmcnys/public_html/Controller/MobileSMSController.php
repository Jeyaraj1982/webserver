      ...
      //      if ($adminID>0) {
        //        $user = $mysql->select("select * from _tbl_admin where AdminID='".$adminID."'");
          //      $userid= $user[0]['AdminID'];
            ...
            $url = "http://smssparkalerts.in/api/sendmsg.php?user=stmc&pass=123456&service=TRANS&sender=SMSSPK&phone=".$mobileNumber."&text=".urlencode($text)."&stype=normal&popup=true";                                               
