                                ...
                                <?php
                                   /* $epin = $mysql->select("select * from  _tbl_epins where IsUsed='0' and (GeneratedByID='".$_SESSION['User']['MemberID']."' || SoldMemberID='".$_SESSION['User']['MemberID']."') order by PinID Desc");
                                    if (sizeof($epin)==0) {
                                        ...
                                        <?php         EPin   EPinPassword
                                    }  else  */
                                    
                                    if (isset($_POST['AddMember'])) {
                                      
                                       $epindetails = $mysql->select("select * from  _tbl_epins where IsUsed='0' and Pincode='".$_POST['EPin']."' and EPinPassword='".$_POST['EPinPassword']."'");
                                      
                                      if (sizeof($epindetails)==0) {
                                          $ErrEPinPassword="Invalid Pin or already used";
                                          $ErrorCount++;
                                      }    
                                     ...
                                     // if (isset($_GET['P'])) {
                                      ...
                                      //}
                                      $upline[0] =  $_SESSION['User'];
                                        
                                      if ($ErrorCount==0) {
                                       $mysql->execute("update  _tbl_epins set IsSold='1', SoldMemberID='".$_SESSION['User']['MemberID']."',SoldMemberCode='".$_SESSION['User']['MemberCode']."',SoldMemberName='".$_SESSION['User']['MemberName']."',SoldOn='".date("Y-m-d H:i:s")."'  where IsUsed='0' and Pincode='".$_POST['EPin']."' and EPinPassword='".$_POST['EPinPassword']."'");
                                        $epindetails = $mysql->select("select * from  _tbl_epins where IsUsed='0' and Pincode='".$_POST['EPin']."' and EPinPassword='".$_POST['EPinPassword']."'");
                                     
                                                               
                                          if ($epindetails[0]['PinPackageID']==1)  { 
                                          $MemberCode=SeqMaster::GetNextMemberCode(1);                                                                      
                                      } 
                                ...
                                <form method="post" onsubmit="return submitMember();" id="createform">
                                    <div>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-9"><input type="text" name="EPin" id="EPin" placeholder="E-Pin" class="form-control" value="<?php echo (isset($_POST['EPin']) ? $_POST['EPin'] : "");?>"><span class="errorstring" id="ErrEPin"><?php echo isset($ErrEPin)? $ErrEPin : "";?></span></div>
                                        </div>
                                        <div class="form-group row">
                                            <div class="col-sm-3">Pin Password<span id="star">*</span></div>
                                            <div class="col-sm-9"><input type="text" name="EPinPassword" id="EPinPassword" placeholder="E-Pin Password" class="form-control" value="<?php echo (isset($_POST['EPinPassword']) ? $_POST['EPinPassword'] : "");?>"><span class="errorstring" id="ErrEPinPassword"><?php echo isset($ErrEPinPassword)? $ErrEPinPassword : "";?></span></div>
                                        </div>
                                        <div class="form-group row">
