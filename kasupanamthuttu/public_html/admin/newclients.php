...
<h5 style="text-align: left;font-family: arial;">New Users</h5> 
...
<?php // $data = $mysql->select("select * from _clients where isblock=-1 and isactive=0 order by id desc"); ?>
  ...
  if (isset($_POST['distName'])) {
    ...
    $data = $mysql->select("select * from _clients where LOWER(trim(streetname)) like '%".$distName."%' and isblock=-1 and isactive=0 order by id desc"); 
  } else {
    if ($_SESSION['country']=="Malaysia") {
        $data = $mysql->select("select * from _clients where isblock=-1 and isactive=0 and country='malaysia' order by id desc");     
    } else {
        $data = $mysql->select("select * from _clients where isblock=-1 and isactive=0 order by id desc");     
    }
}

if ( $_SESSION['user']['role']=="erode" ) {
      $data = $mysql->select("select * from _clients where lcase(streetname) like '%erode%' and isblock=-1 and isactive=0 order by id desc");     
}
...
<table style="font-family:arial;font-size:12px;" width="100%" cellpadding="3" cellspacing="0">
    ...
    <?php foreach($data as $d) { ?>
        <tr>
              ...
              <td style="padding-top:12px;border-bottom:1px solid #ccc;text-align: center"><form action="editclient" method="post"><input type="hidden" value="<?php echo $d['id'];?>" name="clientid"><input type="submit"  class="btn btn-success btn-sm" value="..." ></form></td>
        </tr>
    <?php } ?>
</table>
