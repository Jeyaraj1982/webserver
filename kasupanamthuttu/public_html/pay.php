...
$userinformation = $mysql->select("select * from _clients where md5(concat('x',id))='".$_POST['record']."'");
$jobtypes = $mysql->select("select * from _tbl_JobTypes where JobTypeID='". $_POST['JobType']."'");
if (sizeof($jobtypes)==0)  {
    echo "Error";
    exit;
}
$amount = $jobtypes[0]['JobTrainingFee'];
$paymentfor = strtolower("membership_".str_replace(" ","_",$jobtypes[0]['JobName']));
           
$paymentid = $mysql->insert("_tblPayments",array("TxnDate"    => date("Y-m-d H:i:s"),
                                                 "TxnAmount"  => $amount,
                                                 "ClientID"   => $userinformation[0]['id'],
...
if (isset($_POST['paymentroute']) && $_POST['paymentroute']==1) {
    ...
    $posted['firstname']=$userinformation[0]['firstname'];
    $posted['email']=$userinformation[0]['emailid'];
    $posted['phone']=$userinformation[0]['mobileno'];
    $posted['productinfo']="";
    $posted['surl']='https://www.kasupanamthuttu.com/Payment';
    $posted['furl']='https://www.kasupanamthuttu.com/Payment';
