                                ...
                                <div class="table-responsive">
                                    <table id="myTable" class="table table-striped">
                                        ...
                                        <tbody>
                                            <?php 
                                                if( $_GET['Status']=="All"){
                                                    $sql= $mysql->select("select * from `_tbl_form_16` where FormByID='".$_SESSION['User']['MemberID']."' order by id desc");
                                                }
                                                if( $_GET['Status']=="Submitted"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where InProccess='0' and FormByID='".$_SESSION['User']['MemberID']."' order by id desc"); 
                                                }
                                                if( $_GET['Status']=="Accepted"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where InProccess='1' and FormByID='".$_SESSION['User']['MemberID']."' order by id desc");  
                                                }
                                                if( $_GET['Status']=="InProccessed"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where IsApproved='1' and Completed='0' and FormByID='".$_SESSION['User']['MemberID']."' order by id desc");  
                                                }
                                                if( $_GET['Status']=="Completed"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where Completed='1' and FormByID='".$_SESSION['User']['MemberID']."' order by id desc");  
                                                }
                                                if( $_GET['Status']=="Rejected"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where IsRejected='1' and FormByID='".$_SESSION['User']['MemberID']."' order by id desc");  
                                                }
                                            ?>
                                            <?php 
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo printDateTime($form16['CreatedOn']);?></td>
                                                <td>
                                                ...
                                                <td>
                                                    <?php if($form16['InProccess']==0){ echo printDateTime($form16['CreatedOn']); } ?>
                                                    <?php if($form16['InProccess']==1){ echo printDateTime($form16['InProccessOn']); } ?>
                                                    <?php if($form16['IsApproved']==1 && $form16['Completed']==0){ echo printDateTime($form16['VerifiedOn']); } ?>
                                                    <?php if($form16['Completed']==1){ echo printDateTime($form16['InProcessCompleteOn']); } ?>
                                                    <?php if($form16['IsRejected']==1 && $form16['Completed']==0){ echo printDateTime($form16['VerifiedOn']); } ?>
                                                </td>
                                                <td>
                                                    <?php if ($form16['IsPaid']==0) {?>
                                                        <a href="dashboard.php?action=Paynow&req=<?php echo md5($form16['id'].$form16['OrderNumber']);?>" class="btn btn-warning btn-sm">Pay Now</a>
                                                    <?php } else { ?>
                                                        Paid
                                                    <?php } ?>
                                                </td>
