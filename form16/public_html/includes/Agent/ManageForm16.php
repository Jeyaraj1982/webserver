                                ...
                                <div class="table-responsive">
                                    <table id="myTable" class="table table-striped">
                                        ...
                                        <tbody>
                                            <?php 
                                                if( $_GET['Status']=="All"){
                                                    $sql= $mysql->select("select * from `_tbl_form_16` where AgentID='".$_SESSION['User']['AgentID']."'");
                                                }
                                                if( $_GET['Status']=="Submitted"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where InProccess='0' and AgentID='".$_SESSION['User']['AgentID']."'"); 
                                                }
                                                if( $_GET['Status']=="Accepted"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where InProccess='1' and AgentID='".$_SESSION['User']['AgentID']."'");  
                                                }
                                                if( $_GET['Status']=="InProccessed"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where IsApproved='1' and Completed='0' and AgentID='".$_SESSION['User']['AgentID']."'");  
                                                }
                                                if( $_GET['Status']=="Completed"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where Completed='1' and AgentID='".$_SESSION['User']['AgentID']."'");  
                                                }
                                                if( $_GET['Status']=="Rejected"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where IsRejected='1' and AgentID='".$_SESSION['User']['AgentID']."'");  
                                                }
                                            ?>
                                            <?php 
