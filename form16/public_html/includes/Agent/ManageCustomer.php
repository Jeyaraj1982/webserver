<?php       
    if( $_GET['Status']=="All"){
        $sql= $mysql->select("select * from `_tbl_Members` where `IsCustomer`='1' order by `MemberID` desc ");
        $title="All Customers";
    }
    if( $_GET['Status']=="Active"){
        $sql= $mysql->select("select * from `_tbl_Members` where IsActive='1' and `IsCustomer`='1' order by `MemberID` desc ");
        $title="Active Customers";
    }
    if( $_GET['Status']=="Inactive"){
        $sql= $mysql->select("select * from `_tbl_Members` where IsActive='0' and `IsCustomer`='1' order by `MemberID` desc ");
        $title="Inactive Customers";
    }
    if( $_GET['Status']=="Verified"){
        $sql= $mysql->select("select * from `_tbl_Members` where and  IsMobileNumberVerified='1' `IsCustomer`='1' order by `MemberID` desc ");
                    ...
                    <div class="card">
                        <div class="card-header" style="padding-top:10px;padding-bottom:10px">
                            <div class="card-title">
                                <?php echo $title; ?>
                                <a href="dashboard.php?action=CreateCustomer" style="float:right;color:#fff" class="btn btn-warning btn-sm">Create Customer</a>
                            </div>
                        </div>
                        <div class="card-body">
