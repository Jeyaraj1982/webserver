                                ...
                                <div class="table-responsive">
                                    <table id="myTable" class="table table-striped">
                                        ...
                                        <tbody>
                                            <?php 
                                                if( $_GET['Status']=="All"){
                                                    $sql= $mysql->select("select * from `_tbl_form_16` order by id desc");
                                                }
                                                if( $_GET['Status']=="Submitted"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where InProccess='0'  order by id desc"); 
                                                }
                                                if( $_GET['Status']=="Accepted"){
                                                     $sql= $mysql->select("select * from `_tbl_form_16` where InProccess='1'  order by id desc");
                                                }
                                                if( $_GET['Status']=="InProccessed"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where IsApproved='1' and Completed='0'   order by id desc");  
                                                }
                                                if( $_GET['Status']=="Completed"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where Completed='1'  order by id desc");  
                                                }
                                                if( $_GET['Status']=="Rejected"){
                                                   $sql= $mysql->select("select * from `_tbl_form_16` where IsRejected='1' and Completed='0'  order by id desc");  
                                                }
                                            ?>
                                            <?php 
                                              foreach($sql as $form16){
                                               $Member= $mysql->select("select * from `_tbl_Members` where MemberCode='".$form16['FormByCode']."'");      
                                            ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="dropdown dropdown-kanban" style="float: right;">
                                                        ...
                                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                            ...
                                                            <a class="dropdown-item" href="dashboard.php?action=ViewMember&MCode=<?php echo $form16['FormByCode'];?>" draggable="false"><?php echo ($Member[0]['IsCustomer']==1) ? 'View Customer' : 'View Member';?></a>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
