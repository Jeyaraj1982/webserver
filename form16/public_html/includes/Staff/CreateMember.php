<?php 
    ...
    if (isset($_POST['BtnSubmit'])) {
        ...
        if (!(strlen(trim($_POST['CustomerName']))>0)) {
            $ErrCustomerName ="Please Enter Member Name";
            $Error++;
        }
        if (!(strlen(trim($_POST['CustomerMobileNumber']))>0)) {
            $ErrCustomerMobileNumber = "Please Enter Member Mobile Number";                                      
            $Error++;
        }
        ...
        if (sizeof($dataA)>0) {
            $ErrCustomerMobileNumber = "Member Mobile Number Already Exists";
            $Error++;
        }
         $dataA = $mysql->select("select * from `_tbl_Members` where `EmailID`='".$_POST['CustomerEmailID']."'");
        if (sizeof($dataA)>0) {
            $ErrCustomerEmailID = "Member Email ID Already Exists";
            $Error++;
        }
        ...
        if($Error==0){
            $MemberCode   = SeqMaster::GetNextMemberCode();
                ...
                if(sizeof($Mid)>0){
                    echo "<script>location.href='dashboard.php?action=MemberCreated';</script>";
                }
            }                                                                                                                     
        } 
?>
<script>
    $(document).ready(function () {
        ...
        $("#CustomerName").blur(function () {
            if(IsNonEmpty("CustomerName","ErrCustomerName","Please Enter Member Name")){
                IsAlphabet("CustomerName","ErrCustomerName","Please Enter Alphabet Characters Only");
            }
        });
        $("#CustomerMobileNumber").blur(function () {
            if(IsNonEmpty("CustomerMobileNumber","ErrCustomerMobileNumber","Please Enter Member Mobile Number")){
        ...
        $("#CustomerEmailID").blur(function () {
            if(IsNonEmpty("CustomerEmailID","ErrCustomerEmailID","Please Enter Member Email ID")){
                            ...
                            <div class="card">
                                ...
                                <div class="card-body"> 
                                    <div class="form-group form-show-validation row">
                                        <label for="CustomerName" class="col-sm-3" style="font-weight:normal">Member Name <span class="required-label">*</span></label>
                                        <div class="col-sm-5">
                                            <input type="text" class="form-control" id="CustomerName" name="CustomerName" value="<?php echo (isset($_POST['CustomerName']) ? $_POST['CustomerName'] :"");?>"Placeholder="Member Name">
                                            <span class="errorstring" id="ErrCustomerName"><?php echo isset($ErrCustomerName)? $ErrCustomerName : "";?></span>
                                        </div>
                                    </div>
                                    <div class="form-group form-show-validation row">
                                        <label for="CustomerMobileNumber" class="col-sm-3" style="font-weight:normal">Member Mobile Number <span class="required-label">*</span></label>
                                        <div class="col-sm-5">
                                            <input type="text" class="form-control" id="CustomerMobileNumber" maxlength="10" name="CustomerMobileNumber" value="<?php echo (isset($_POST['CustomerMobileNumber']) ? $_POST['CustomerMobileNumber'] :"");?>"Placeholder="Member Mobile Number">
                                            <span class="errorstring" id="ErrCustomerMobileNumber"><?php echo isset($ErrCustomerMobileNumber)? $ErrCustomerMobileNumber : "";?></span>
                                        </div>
                                    </div>
                                    <div class="form-group form-show-validation row">
                                        <label for="CustomerEmailID" class="col-sm-3" style="font-weight:normal">Member Email ID <span class="required-label">*</span></label>
                                        <div class="col-sm-5">
                        ...
                        <div class="col-md-12"> 
                                    <div class="card-action">
                                        <div class="row">
                                            ...
                                            <a href="dashboard.php?action=ManageMember&Status=All" class="btn btn-outline-primary">Cancel</a> &nbsp;
                                            <button type="submit" class="btn btn-primary" name="BtnSubmit" id="BtnSubmit">Create Memeber</button>
                                            </div>                                        
                                        </div>                                                                             
                                    </div>
