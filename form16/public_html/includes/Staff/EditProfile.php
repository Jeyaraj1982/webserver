<?php 
  $sql= $mysql->select("select * from `_tbl_Admin` where  `AdminCode`='".$_SESSION['User']['AdminCode']."'");
if (isset($_POST['BtnSubmit'])) {
          ...
          if (!(strlen(trim($_POST['Name']))>0)) {
             ...
             $data = $mysql->select("select * from `_tbl_Admin` where  `MobileNumber`='".$_POST['MobileNumber']."' and `AdminCode`<>'".$sql[0]['AdminCode']."'");
             if (sizeof($data)>0) {
                 $ErrMobileNumber = "Mobile Number Already Exists";
                 $Error++;
             }
             $dataE = $mysql->select("select * from `_tbl_Admin` where  `EmailID`='".$_POST['EmailID']."' and `AdminCode`<>'".$sql[0]['AdminCode']."'");
             if (sizeof($dataE)>0) {
                 $ErrEmailID = "EmailID Already Exists";
                 $Error++;
             }
             if($Error==0){
                 $mysql->execute("update `_tbl_Admin` set `AdminName`='".$_POST['Name']."',
                                                            ...
                                                            `Pincode`='".$_POST['PinCode']."' where `AdminCode`='".$sql[0]['AdminCode']."'");
