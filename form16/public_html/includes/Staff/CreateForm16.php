<?php 
    ...
    if (isset($_POST['BtnSubmit'])) {
        ...
        if (!(strlen(trim($_POST['MemberName']))>0)) {
            $ErrMemberName ="Please Enter Member Name";
            $Error++;
        }
        ...
        if (!(strlen(trim($_POST['MemberMobileNumber']))>0)) {
            $ErrMemberMobileNumber = "Please Enter Member Mobile Number";                                      
            $Error++;
        }
        ...
        $dataA = $mysql->select("select * from `_tbl_Members` where `MemberName`='".$_POST['MemberName']."'");
        if (sizeof($dataA)>0) {
            $ErrMemberName = "Member Name Already Exists";
            $Error++;
        }
        $dataA = $mysql->select("select * from `_tbl_Members` where `MobileNumber`='".$_POST['MemberMobileNumber']."'");
        if (sizeof($dataA)>0) {
            $ErrMemberMobileNumber = "Member Mobile Number Already Exists";
            $Error++;
        }
         $dataA = $mysql->select("select * from `_tbl_Members` where `EmailID`='".$_POST['MemberEmailID']."'");
        if (sizeof($dataA)>0) {
            $ErrMemberEmailID = "Member Email ID Already Exists";
            $Error++;
        }
        ...
        if($Error==0){
            ...
            $target_dir = "uploads/Form16/".$MemberCode."/";
            ...
            if ($err==0) {
                 ...
                 $Mid = $mysql->insert("_tbl_Members",array("MemberName"     => $_POST['MemberName'],
                                                           "MemberCode"     => $MemberCode,
                                                           "MobileNumber"   => $_POST['MemberMobileNumber'],
                ...
                $id = $mysql->insert("_tbl_form_16",array("FinancialYearID"  => $_POST['FinYear'],                          
                                                          ...
                                                          "FormBy"           => $_POST['MemberName'], 
                                                          "Remarks"          => $_POST['Remarks'], 
                                                          "FormByID"         => $Mid, 
                                                          "zip_file"         => $zip_file, 
                                                          "FormByCode"       => $MemberCode, 
                                                          "StaffID"        => $_SESSION['User']['AdminID'],
                                                          "CreatedOn"        => date("Y-m-d H:i:s"))); 
                ...
                if ($id>0) {
                    ...
                    $mysql->insert("_tbl_formlogs",array("ActionPerformed"    => date("Y-m-d H:i:s"),
                                                         ...
                                                         "UpdatedStaffName"   => $_SESSION['User']['AdminName']));
                                                                       ...
                                                                       //$_POST['MemberName']    $_POST['MemberMobileNumber']
                
                //Customer ID                                        
                MobileSMS::sendSMS($_POST['MobileNumber'],SMSTemplate::submitTemplete("Member",$id,$_POST['MemberName'] ),$Mid);
                //Agent SMS
                //MobileSMS::sendSMS($_SESSION['User']['MobileNumber'],SMSTemplate::submitTemplete("Staff",$id,$_POST['MemberName'] ),$_SESSION['User']['AgentID']);
...
<script>
    ...
    $(document).ready(function () {
        $("#MobileNumber").keypress(function(e) {
         ...
         $("#MemberMobileNumber").keypress(function(e) {
            if (e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
                $("#ErrMemberMobileNumber").html("Digits Only").fadeIn().fadeIn("fast");
                return false;
            }
        });
        $("#MemberName").blur(function () {
            if(IsNonEmpty("MemberName","ErrMemberName","Please Enter Member Name")){
                IsAlphabet("MemberName","ErrMemberName","Please Enter Alphabet Characters Only");
            }
        });
        $("#MemberMobileNumber").blur(function () {
            if(IsNonEmpty("MemberMobileNumber","ErrMemberMobileNumber","Please Enter Member Mobile Number")){
                IsMobileNumber("MemberMobileNumber","ErrMemberMobileNumber","Please enter valid mobile number");
            }
        });
        $("#MemberEmailID").blur(function () {
            if(IsNonEmpty("MemberEmailID","ErrMemberEmailID","Please Enter Member Email ID")){
                IsEmail("MemberEmailID","ErrMemberEmailID","Please enter valid Member Email ID");
            }
        });
        $("#Name").blur(function () {
    ...
    function SubmitProfile() {
        ErrorCount=0;    
        $('#ErrMemberName').html("");            
        $('#ErrMemberMobileNumber').html("");
        $('#ErrMemberEmailID').html("");                                                                                                           
        $('#ErrName').html("");            
                            ...
                            <div class="card">
                                ...
                                <div class="card-body"> 
                                    <div class="form-group form-show-validation row">
                                        <label for="MemberName" class="col-sm-3" style="font-weight:normal">Member Name <span class="required-label">*</span></label>
                                        <div class="col-sm-5">
                                    ...
                                    <div class="form-group form-show-validation row">
                                        <label for="MemberMobileNumber" class="col-sm-3" style="font-weight:normal">Member Mobile Number <span class="required-label">*</span></label>
                                        <div class="col-sm-5">
                                            <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text" id="basic-addon1">+91</span>
                                                    </div>
                                                    <input type="text" class="form-control" id="MemberMobileNumber" maxlength="10" name="MemberMobileNumber" value="<?php echo (isset($_POST['MemberMobileNumber']) ? $_POST['MemberMobileNumber'] :"");?>"Placeholder="Member Mobile Number">
                                                </div>
                                            <span class="errorstring" id="ErrMemberMobileNumber"><?php echo isset($ErrMemberMobileNumber)? $ErrMemberMobileNumber : "";?></span>
                                        </div>
                                    </div>
                                    <div class="form-group form-show-validation row">
                                        <label for="MemberEmailID" class="col-sm-3" style="font-weight:normal">Member Email ID <span class="required-label">*</span></label>
                                        <div class="col-sm-5">
                                            <input type="text" class="form-control" id="MemberEmailID" name="MemberEmailID" value="<?php echo (isset($_POST['MemberEmailID']) ? $_POST['MemberEmailID'] :"");?>"Placeholder="Member Email ID">
                                            <span class="errorstring" id="ErrMemberEmailID"><?php echo isset($ErrMemberEmailID)? $ErrMemberEmailID : "";?></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="form-group form-show-validation row">                                                                                                                                                         
                                            ...
                                            <div class="col-sm-4 text-left">
                                                ...
                                                <?php $FinancialYears = $mysql->select("select * from _tblFinancialYears");?>                          
                                                 ...
                                                 <span class="errorstring" id="ErrFinYear"></span>
                        ...
                        <div class="col-md-12"> 
                                    <div class="card-action">
                                        <div class="row">
                                            ...
                                            <a href="dashboard.php?action=ManageMemberForm16&Status=All" class="btn btn-outline-primary">Cancel</a>
                                            <button type="submit" class="btn btn-primary" name="BtnSubmit" id="BtnSubmit">Submit Order</button>
                                            </div>                                        
                                        </div>                                                                             
                                    </div>
