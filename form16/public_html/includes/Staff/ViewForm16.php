<?php 
    ...
    MobileSMS::sendSMS($member[0]['MobileNumber'],"Your form has been inprocessed. Thanks Form16.online",$id);
    ...
    if (isset($_POST['Complete'])) {  
             ...
             if (!is_dir('uploads/Form16/'.$form[0]['FormByCode'].'/Acknowledgement_')) {
                    ...
                    if (isset($_FILES["File"]["name"]) && strlen(trim($_FILES["File"]["name"]))>0 ) {
                        $Acknowledgement = time().$_FILES["File"]["name"];
                        if (!(move_uploaded_file($_FILES["File"]["tmp_name"], 'uploads/Form16/'.$form[0]['FormByCode'].'/Acknowledgement_' . $Acknowledgement))) {
                           $err++;
                    ...
                    if ($err==0) {
                        ...
                        $id=$mysql->execute("update `_tbl_form_16` set `Acknowledgement`     ='".$_POST['File']."',
                                                                       `Completed`          ='1',
                                                                       `InProccess`          ='2',
    ...
    $('#PubplishNow').modal('show'); 
      var content = '<div class="modal-header">'
                    ...
                    + '<div class="modal-footer">'
                        ...
                        + '<button type="button" class="btn btn-warning" name="Create" onclick="SaveRemarks(\''+FromID+'\')" style="font-family:roboto">Add Remarks</button>'
                    + '</div>';
