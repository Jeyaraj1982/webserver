<?php
    ...
    $d = $mysql->select("select * from telegram_subscribers where client_id='".$_GET['clientid']."' and telegram_route='abj'");
    if (sizeof($d)==0) {
        $mobilenumber = str_replace("ABJ","",strtoupper(trim($_GET['message'])));
        if ($mobilenumber>=6000000000 && $mobilenumber<=9999999999) {
            $message = " Welcome to ABJ Service. Any queries or feedback please contact support team.";
            
            $memberTable = $mysql->select("select * from _tbl_member where MobileNumber='".$mobilenumber."' and Mapped='3' ");
