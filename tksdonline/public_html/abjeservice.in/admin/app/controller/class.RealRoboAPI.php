<?php
    class RealRoboAPI {
        ...
        public static function sendRequest($param) {
            ...
            //update api url
            $mysql->execute("update `_tbl_transactions` set  `APIID`='3', `APIName`='realrobo', `calledurl`='".$api_url."'  where `txnid`='".$param['operator']."'");
            
            //call api
            $api_response = getHttp($api_url);
            
            //update api response
            $mysql->execute("update `_tbl_transactions` set `urlresponse`='".$api_response."',`OperatorDate`='".date("Y-m-d H:i:s")."' where `txnid`='".$param['operator']."'");
