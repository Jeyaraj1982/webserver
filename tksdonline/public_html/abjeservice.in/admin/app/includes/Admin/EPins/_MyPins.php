...
if (isset($_GET['filter']) && $_GET['filter']=="all") {
    $records=$mysql->select("SELECT * FROM _tblEpins where CreatedByID='".$_SESSION['User']['AdminID']."' or OwnToID='".$_SESSION['User']['MemberID']."'  ");
    $title = "All Epins";
    $error = "No EPins found";
} elseif (isset($_GET['filter']) && $_GET['filter']=="approved") {
    $records = $mysql->select("select * from _tbl_member_bank_details where MemberID='".$_SESSION['User']['AdminID']."' and `IsApproved`='1' order by RequestID Desc");
    $title = "Unused Epins";
    $error = "No Unused EPins found";
}  elseif (isset($_GET['filter']) && $_GET['filter']=="rejected") {
    $records = $mysql->select("select * from _tbl_member_bank_details where MemberID='".$_SESSION['User']['AdminID']."' and `IsRejected`='1' order by RequestID Desc");
    $title = "Used E-Pins";
    $error = "No Used EPins found";
} else {
    $records=$mysql->select("SELECT * FROM _tblEpins where CreatedByID='".$_SESSION['User']['AdminID']."' or OwnToID='".$_SESSION['User']['AdminID']."'  ");
    $title = "All Epins";
    $error = "No EPins found";
}
?>
