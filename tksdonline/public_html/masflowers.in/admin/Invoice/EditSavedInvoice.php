<?php 
	$invoiceValues  = $mysql->select("SELECT * FROM save_invoice_order WHERE md5(order_id) = '".$_GET['invoice_id']."'");
    $invoiceDate = date("d/m/Y", strtotime($invoiceValues[0]['order_date'])); 
    $invoiceItems = $mysql->select("SELECT * FROM save_invoice_order_item WHERE order_id = '".$invoiceValues[0]['order_id']."'");  
	
	$createdby = $mysql->select("select * from _tbl_sales_admin where AdminID='".$invoiceValues[0]['SavedByID']."'");
	$LastModifiedBy = $mysql->select("select * from _tbl_sales_admin where AdminID='".$invoiceValues[0]['LastModifiedBy']."'");
?>
...
<?php
   ...
   <?php } else {  ?>
<?php 
$cstmr = $mysql->select("select * from _tbl_sales_customers where CustomerID='".$invoiceValues[0]['user_id']."'");
?>
                        ...
                        <div class="container content-invoice">
                            ...
                            <input type="hidden" name="ReturnCashToCustomer" id="ReturnCashToCustomer" class="form-control">
                                <div class="load-animate animated fadeInUp">
                                    ...
                                    <input type="hidden" name="SavedOrderID" id="SavedOrderID" value="<?php echo (isset($_POST['SavedOrderID']) ? $_POST['SavedOrderID'] : $invoiceValues[0]['order_id']);?>">
                                   ...
                                   <div class="row">  
										...
										<div class="col-sm-6">
											Order Number : <?php echo $invoiceValues[0]['order_code'];?><br>
											Order Date : <?php echo date("d M, Y", strtotime($invoiceValues[0]['order_date']));?><br><br>
											Order Created By : <?php echo $createdby[0]['AdminName'];?><br>
											Order Last Modified By : <?php echo $LastModifiedBy[0]['AdminName'];?>
										</div>
                                   </div>                                               
                                    ...
                                    <div class="row">
                                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                            ...
                                            <table class="table table-bordered table-hover" id="invoiceItem">
                                                <?php $i=1; foreach($invoiceItems as $invoiceItem){ ?>
												<?php $AddedBy = $mysql->select("select * from _tbl_sales_admin where AdminID='".$invoiceItem['SavedByID']."'");?>
												<form method="post" id="itemFrm_<?php echo $invoiceItem['order_item_id'];?>">
												<input type="hidden" name="ItemID" id="ItemID" value="<?php echo $invoiceItem['order_item_id'];?>">
												<tr>
													<td width="2%"><input class="itemRow" type="checkbox"></td>          
													<td width="38%"><input type="text" value="<?php echo $invoiceItem['item_name'];?>" readonly="readonly" name="productName" id="productName_<?php echo $i;?>" class="form-control" autocomplete="off" style="height: auto !important;"></td>
													<td width="20%">
														<div class="input-group">
															<input type="text" name="quantity" value="<?php echo $invoiceItem['order_item_quantity'];?>" onkeypress="SwitchamountBox(event)" id="quantity_<?php echo $i;?>" class="form-control quantity" autocomplete="off" style="height: auto !important;text-align:center">
															<div class="input-group-append">
																<div class="input-group-text" id="Unit_<?php echo $i;?>">KG</div>
															</div> 
														</div> 
													</td> 
													<td width="15%" style="text-align:right"><input type="text" value="<?php echo $invoiceItem['order_item_price'];?>" name="price" onkeypress="SwitchproductBox(event)" id="price_<?php echo $i;?>" class="form-control price" autocomplete="off" style="height: auto !important;text-align:right"></td>         
													<td width="15%" style="text-align:right"><input type="text"  value="<?php echo $invoiceItem['order_item_final_amount'];?>" readonly="readonly" name="total" id="total_<?php echo $i;?>" class="form-control total" autocomplete="off" style="height: auto !important;text-align:right"></td>          
													<td width="20%"><?php echo $AddedBy[0]['AdminName'];?></td>          
													<td width="20%"><?php echo date("d M, Y H:i", strtotime($invoiceItem['AddedOn']));?></td>          
													<td><span onclick="callconfirmationupdate('<?php echo $invoiceItem['order_item_id'];?>')"><i class="fas fa-undo-alt"></i></span></td>
													<td><span onclick="Removecallconfirmation('<?php echo $invoiceItem['order_item_id'];?>')"><i class="fas fa-trash"></i></span></td>
												</tr>
												</form>
												<?php $i++;} ?>
												<!--<tr>
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-4">
                                                        <div class="input-group">
                                                            ...
                                                            <input type="text" value="<?php echo (isset($_POST['subTotal']) ? $_POST['subTotal'] :$invoiceValues[0]['order_total_after_tax']);?>" class="form-control" name="subTotal" id="subTotal" placeholder="Subtotal" readonly="readonly" style="text-align: right;">
                                                        </div>
    ...
    if(Sale=="Cash"){
     ...
     if(ErrorCount==0) {
       ...
       var loading = "<div style='padding:80px;text-align:center;color:#aaa'><img src='https://masflowers.in/admin/assets/loading.gif'  style='width:80px'><br>Processing ...</div>";                                                                                
            ...
            $.post( "webservice.php?action=MoveCreateInvoice", param,function(data) {
				var obj = JSON.parse(data); 
                var html = "";
				if (obj.status=="failure") {    
                    html = "<div style='width:100%;padding:10px;background:#fff;text-align:center;color:red'><img src='https://masflowers.in/admin/assets/accessdenied.png' style='width:128px'><br><br>Transaction failed.<br>"+obj.message;
...
<script>
function CallConfirmationUpdateInvoice(){
    ErrorCount=0; 
    ...
    if(ErrorCount==0) {
            var txt = '<div class="form-group row">'                                                                     
               ...
               +'<div class="form-group row">'                                                          
                ...
                +'<div style="padding:20px;text-align:center">'
                    +'<button type="button" class="btn btn-outline-success" data-dismiss="modal" >Cancel</button>&nbsp;&nbsp;&nbsp;'
                    +'<button type="button" class="btn btn-success" onclick="UpdateInvoice()" >Yes, Confirm</button>'
                 +'</div>';  
...
function UpdateInvoice() {  
    ...
    if(ErrorCount==0) {
       ...
       var loading = "<div style='padding:80px;text-align:center;color:#aaa'><img src='https://masflowers.in/admin/assets/loading.gif'  style='width:80px'><br>Processing ...</div>";                                                                                
            ...
            $.post( "http://masflowers.in/user/webservice.php?action=UpdateInvoice", param,function( data ) {
                var obj = JSON.parse(data); 
                var html = "";
                if (obj.status=="failure") {    
                    html = "<div style='width:100%;padding:10px;background:#fff;text-align:center;color:red'><img src='https://masflowers.in/admin/assets/accessdenied.png' style='width:128px'><br><br>Transaction failed.<br>"+obj.message;
                    ...
                    html = '<div class="form-group row">'
                            ...
                            +'</div>'
                            +'<div style="padding:20px;text-align:center">'
                                +'<button type="button" class="btn btn-outline-success" onclick="location.href=\'\'" >Countinue</button>'
                            +'</div>';                                                                   
                    $('#ConfirmationPopup').modal("show");                                                                        
                }
                $("#xconfrimation_text").html(html);
...
function callconfirmationupdate(ItemID){
   ...
   // $("#xconfrimation_text").html(loading);
    $.post( "http://masflowers.in/admin/webservice.php?action=updateorderfromadmin",param,function(data) {                  
        var obj = JSON.parse(data); 
        var html = "";                                                                              
        if (obj.status=="failure") {
...
function Removecallconfirmation(ItemID){
 ...
 function Removeitem(ItemID) {
     var param = $( "#itemFrm_"+ItemID).serialize();
   // $("#xconfrimation_text").html(loading);                               
    $.post( "http://www.masflowers.in/admin/webservice.php?action=Removeitem",param,function(data) {                  
        var obj = JSON.parse(data); 
        var html = "";                                                                              
        if (obj.status=="failure") {
            html = "<div class='form-group row'><div class='col-sm-12' style='text-align:center'><img src='../assets/accessdenied.png' style='width:128px'><br>"+obj.message+"<br></div></div>";
