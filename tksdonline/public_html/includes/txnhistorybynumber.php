...
<?php   
 ...
 if(isset($_POST['viewTransaction'])) { 
    ...
    if (isset($_GET['operator'])) {
        ...
        $summary = $mysql->select("select * from `_tbl_transactions` where `operatorcode`='".$_GET['operator']."' and rcnumber like '%".$_POST['contactdetails']."%' and `MemberID`='".$_SESSION['User']['MemberID']."' order by txnid desc");
        $optttitle= $opt[0]['OperatorTypeCode']." <span style='color:green'>(".$opt[0]['OperatorName'].")</span><br>";
    } else {   
        $summary = $mysql->select("select * from _tbl_transactions where MemberID='".$_SESSION['User']['MemberID']."' and rcnumber like '%".$_POST['contactdetails']."%' order by txnid desc");
        $optttitle = "";
    }
 }  else {
     if (isset($_GET['operator'])) {
        ...
        $summary = $mysql->select("select * from _tbl_transactions where MemberID='".$_SESSION['User']['MemberID']."' order by txnid desc");
        $optttitle = "";
    } 
 }
