...
// 4 : comission debit from admin
    if (isset($_POST['refillBtn'])) {
        ...
        if (!($app->getBalance($member[0]['MemberID'])<=$_POST['Amount'])) {
            $err++;
            $errormsg = "Dealer balance is not sufficiant to debit";
        }
        if ($error==0) {
             
             $description = $_POST['Description'];
            $id=$mysql->insert("_tbl_accounts",array("MemberID"    => $member[0]['MemberID'],
                                                     "Particulars" => 'Debit/FromAdmin/Dealer:'.$member[0]['MobileNumber']."/".$description,                    
                                                     "Credit"      => "0",                    
                                                     "Debit"       => $_POST['Amount'], 
             ...
             $id=$mysql->insert("_tbl_accounts",array("MemberID"    => $_SESSION['User']['MemberID'],
                                                     "Particulars" => 'Debit/ToAdmin/Dealer:'.$member[0]['MobileNumber']."/".$description,                   
                                                     "Credit"      => $_POST['Amount'],                    
                                                     "Debit"       => "0", 
             ...
             MobileSMS::sendSMS($member[0]['MobileNumber'],"Dear Dealer, Your wallet has debited Rs. ".$_POST['Amount'].".   Wallet Balance Rs.".number_format($app->getBalance($member[0]['MemberID']),2),$member[0]['MemberID']);
            ...
            <script>
             $(document).ready(function() {
                swal("Reversed Successfully", {
                    ...
                    location.href='dashboard.php?action=Dealers/Reverse'
                });
                 });
            </script>
        <?php } else { ?>
                    ...
                    <div class="row mb15">
                        <div class="col-md-12 col-xs-6 b-r">
                            ...
                            <br>
                             ...
                             <option value="0">Slect Dealer</option>
