...
<div class="row" style="margin-bottom:0px">
    ...
    $summary = $mysql->select("select * from `_tbl_accounts` where (date(TxnDate)>=date('".$fdate."') and date(TxnDate)<=date('".$tdate."') ) and  `MemberID`='".$_SESSION['User']['MemberID']."' order by AccountID desc");
    if (sizeof($summary)==0) {
...
<table class="table table-striped">
  ...
  <?php
    foreach($summary as $s) {
        if($s['CreditTo']>0) {
            $CreditTo = $mysql->select("select * from _tbl_Members where MemberID='".$s['CreditTo']."'");
        }
        ?>
        <tr>
            ...
            <?php if($s['CreditTo']>0) { echo "<b>".$CreditTo[0]['MemberName']."</b>";?><Br>  <?php } ?>
