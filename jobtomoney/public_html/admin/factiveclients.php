...
<?php 
if ($_GET['f']=="all") {
 $data = $mysql->select("select * from _clients where isblock=0 and isactive=1 and trim(streetname)='".trim($_SESSION['user']['AssignedDistrict'])."' order by id desc");          
 $title = "All Users"   ;
} elseif ($_GET['f']=="emailjob") {
$data = $mysql->select("select * from _clients where isblock=0 and isactive=1 and emailjob_enabled='1' and trim(streetname)='".trim($_SESSION['user']['AssignedDistrict'])."'  order by id desc");          
$title = "Email Job Users"   ;
} elseif ($_GET['f']=="adposting") {
$data = $mysql->select("select * from _clients where isblock=0 and isactive=1 and adposting_enabled='1'  and trim(streetname)='".trim($_SESSION['user']['AssignedDistrict'])."' order by id desc");          
$title = "AdPosting Job Users"   ;
} elseif ($_GET['f']=="smsjob") {
$data = $mysql->select("select * from _clients where isblock=0 and isactive=1 and smsjob_enabled='1' and  trim(streetname)='".trim($_SESSION['user']['AssignedDistrict'])."' order by id desc");          
$title = "SMS Job Users"   ;
} else {
 $data = $mysql->select("select * from _clients where isblock=0 and isactive=1 and trim(streetname)='".trim($_SESSION['user']['AssignedDistrict'])."' order by id desc");          
 $title = "All Users"   ;
