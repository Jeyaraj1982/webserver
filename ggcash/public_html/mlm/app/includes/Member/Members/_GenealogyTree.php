...
<?php 
    ...
    if (sizeof($A)==0) {
        ...
        $str .= "[{v:'".$A[0]['ChildCode']."', f:'<div id=id_".$A[0]['ChildCode']."  class=".getCss($A[0]['PackageID'])." >".getImage($A[0]['Thumb'],$A[0]['Sex'])."<br><b><span style=color:green;>".$A[0]['ChildCode']."</span></b><br>".$A[0]['ChildName']."<div>[<a href=dashboard.php?action=Members/GenealogyTree&Code=".$A[0]['ChildCode']." >Downline</a>]</div></div>'},'".$L[0]['MemberCode']."', 'Active'],";
        
        $AA = $mysql->select("select * from `_tblPlacements` where `ParentCode`='".$A[0]['ChildCode']."' and Position='L'");
        if (sizeof($AA)==0) {
            if ($leftCode==$A[0]['ChildCode']) { 
                $str .= "[{v:'Vacant_1582', f:'<br>".getImage($_A[0]['Thumb'],$_A[0]['Sex'])."<div style=color:red;><a href=dashboard.php?action=Members/CreateMem&Code=".$A[0]['ChildCode']."&p=1 target=new>Add User</a></div><div style=width:80px;></div><br>'},'".$A[0]['ChildCode']."', 'GCCHub'],";
            } else {
        ...
        if (sizeof($AB)==0) {
             if ($rightCode==$A[0]['ChildCode']) {
                $str .= "[{v:'Vacant_1583', f:'<br><div style=color:red;><a href=dashboard.php?action=Members/CreateMem&Code=".$A[0]['ChildCode']."&p=2 target=new>Add User</a></div><div style=width:80px;></div><br>'},'".$A[0]['ChildCode']."', 'GCCHub'],";
             } else {
    ...
    if (sizeof($B)==0) { 
        ...
        $str .= "[{v:'".$B[0]['ChildCode']."', f:'<div id=id_".$B[0]['ChildCode']."  class=".getCss($B[0]['PackageID'])." >".getImage($_B[0]['Thumb'],$_B[0]['Sex'])."<br><b><span style=color:green;>".$B[0]['ChildCode']."</span></b><br>".$B[0]['ChildName']."<div>[<a href=dashboard.php?action=Members/GenealogyTree&Code=".$B[0]['ChildCode']." >Downline</a>]</div></div>'},'".$L[0]['MemberCode']."', 'Active'],";
        
        $BA = $mysql->select("select * from `_tblPlacements` where `ParentCode`='".$B[0]['ChildCode']."' and Position='L'");
        if (sizeof($BA)==0) {
             if ($leftCode==$B[0]['ChildCode']) {
                $str .= "[{v:'Vacant_1585', f:'<br>".getImage($B[0]['Thumb'],$_B[0]['Sex'])."<div style=color:red;><a href=dashboard.php?action=Members/CreateMem&Code=".$B[0]['ChildCode']."&p=1 target=new>Add User</a></div><div style=width:80px; font-style:italic>&nbsp;</div><div style=width:80px;></div><br>'},'".$B[0]['ChildCode']."', 'GCCHub'],";
             } else {
        ...
        if (sizeof($BB)==0) {
             if ($rightCode==$B[0]['ChildCode']) {
                $str .= "[{v:'Vacant_1586', f:'<br>".getImage($B[0]['Thumb'],$_B[0]['Sex'])."<div style=color:red;><a href=dashboard.php?action=Members/CreateMem&Code=".$B[0]['ChildCode']."&p=2 target=new>Add User</a></div><div style=width:80px; font-style:italic>&nbsp;</div><div style=width:80px;></div><br>'},'".$B[0]['ChildCode']."', 'GCCHub'],";
             } else {              
                  ...
                  ?>
                    <table align="right" class="table-bordered">
                            ...
                            <tr style="background:blue;color:#fff;">
                                <td colspan="3"  style="text-align: center;padding:5px 10px !important"><?php echo $parentCode;?></td>
                            </tr>
                             <tr>
                                <td></td>
                            ...
                            <tr>
                                <td>PV</td>
                                <td style="text-align:right ;"><?php echo $member_leftPV;?></td>
                                <td style="text-align:right ;"><?php echo $member_rightPV;?></td>
                            </tr>
                            <tr>
                                <td>BV</td>
