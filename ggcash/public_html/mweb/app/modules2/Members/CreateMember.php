<?php
    ...
    if (isset($_POST['submitBtn'])) {
        ...
        if ($error==0) {
            ...
            if ($_POST['pos']=="L") {
                $temp = explode(",",GetLeft($_SESSION['User']['MemberCode'],"L"));
                $uplineID = $temp[0];
                if (isset($_POST['LSubPos']) && strlen($_POST['LSubPos'])==1) {
                     $pos = $_POST['LSubPos'];
                } else {
                    $pos = "L";
                }
            ...
            if ($_POST['pos']=="R") {
                $temp = explode(",",GetRight($_SESSION['User']['MemberCode'],"R"));
                $uplineID = $temp[0];
                if (isset($_POST['RSubPos']) && strlen($_POST['RSubPos'])==1) {
                     $pos = $_POST['RSubPos'];
                } else {
                    $pos = "R";
                }
            ...
            $upline = $mysqldb->select("select * from _tbl_Members where MemberCode='".$uplineID."'");
            ...
            if ($_SESSION['User']['MemberID']==$upline[0]['MemberID']) {
                MobileSMS::sendSMS("Dear ".$_SESSION['User']['MemberName'].", Your have created a member ".$_POST['MemberName']."(".$MemberCode.") on ".date("M d, Y")."  ",$_SESSION['User']['MemberID']);
            } else {
                MobileSMS::sendSMS("Dear ".$_SESSION['User']['MemberName'].", Your have created a member ".$_POST['MemberName']."(".$MemberCode.") on ".date("M d, Y")."  ",$_SESSION['User']['MemberID']);
                $up = $mysqldb->select("select * from _tbl_Members where MemberID='".$upline[0]['MemberID']."'");
                MobileSMS::sendSMS("Dear ".$up[0]['MemberName'].", ".$_SESSION['User']['MemberCode']." has created new member and placed under you. Member Information  ".$_POST['MemberName']." (".$MemberCode.") on ".date("M d, Y")."  ",$up[0]['MemberID']);
            }
