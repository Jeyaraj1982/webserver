...
<?php
 /*$Requests  = $mysqldb->select("SELECT *
                        ...
                        ON _tbl_wallet_request_epin.MemberID=_tbl_Members.MemberID order by _tbl_wallet_request_epin.RequestID desc" );*/
...
if (isset($_GET['filter']) && $_GET['filter']=="all") {
  ...
  $Requests  = $mysqldb->select("SELECT *
       ...
       $Requests  = $mysqldb->select("SELECT *
                        FROM `_tbl_wallet_request_epin`
                        LEFT  JOIN `_tbl_Members`  
                        ON `_tbl_wallet_request_epin`.`MemberID`=`_tbl_Members`.`MemberID` where `_tbl_wallet_request_epin`.`IsApproved`='1' order by `_tbl_wallet_request_epin`.`RequestID` desc"); 
}  elseif (isset($_GET['filter']) && $_GET['filter']=="rejected")  {
       $Requests  = $mysqldb->select("SELECT *
                        FROM `_tbl_wallet_request_epin`
                        LEFT  JOIN `_tbl_Members`  
                        ON `_tbl_wallet_request_epin`.`MemberID`=`_tbl_Members`.`MemberID` where `_tbl_wallet_request_epin`.`IsRejected`='0' order by `_tbl_wallet_request_epin`.`RequestID` desc"); 
                        
}elseif (isset($_GET['filter']) && $_GET['filter']=="pending")  {
       $Requests  = $mysqldb->select("SELECT *
                        FROM `_tbl_wallet_request_epin`
                        LEFT  JOIN `_tbl_Members`  
                        ON `_tbl_wallet_request_epin`.`MemberID`=`_tbl_Members`.`MemberID` where (`_tbl_wallet_request_epin`.`IsApproved`='0' and `_tbl_wallet_request_epin`.`IsRejected`='0') order by `_tbl_wallet_request_epin`.`RequestID` desc"); 
} else {
       $Requests  = $mysqldb->select("SELECT *
                        FROM `_tbl_wallet_request_epin`
                        LEFT  JOIN `_tbl_Members`  
