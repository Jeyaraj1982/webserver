...
<div class="card">
 ...
 $mysqldb   = new MySqldatabase("localhost","ggcash_user","ggcash_user","ggcash_database");
                       ...
                       if (isset($_POST['submitpassword'])) {
                          echo "select * from _tbl_admin where AdminID='".$_SESSION['User']['AdminID']."'";                        
                       $getpassword =$mysqldb->select("select * from _tbl_admin where AdminID='".$_SESSION['User']['AdminID']."'");
                        print_r($getpassword);
                            if ($getpassword[0]['AdminPassword']!=$_POST['CPassword']) {
                                echo "<span style='color:red'>Incorrect Current password.</span>";
                               
                            } 
                            if ($getpassword[0]['AdminPassword']==$_POST['CPassword']) {
                                $sqlQry = "update `_tbl_admin` set `AdminPassword`='".$_POST['CNPassword']."' where AdminID='".$_SESSION['User']['AdminID']."'";
